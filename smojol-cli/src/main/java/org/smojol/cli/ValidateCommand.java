...
public class ValidateCommand implements Callable<Integer> {
  ...
  public Integer call() {
    ...
    LOGGER.info(
        String.format("Running validation in %s mode...", isStrict ? "STRICT" : "PERMISSIVE"));
    return new ValidateTaskRunner(programSearch)
            .processPrograms(
                programNames,
                sourceDir,
                LanguageDialect.valueOf(dialect),
                copyBookPaths,
                dialectJarPath,
                outputPath,
                isStrict
                    ? ProgramValidationErrors::IS_COMPLETE_SUCCESS
