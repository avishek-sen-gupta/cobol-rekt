...
public class DecimalPointAligner implements DataFilter {
  ...
  public String filter(String s) {
    ...
    return new LeftAdjuster(numIntegerDigits, ZonedDecimalDataTypeSpec.CHARACTER_ZERO).filter(lhs)
        + new RightAdjuster(numFractionalDigits, ZonedDecimalDataTypeSpec.CHARACTER_ZERO)
            .filter(rhs);
  }
  ...
  //        String lhs = decimalPointPosition != -1 ? s.substring(0, decimalPointPosition) : s;
  //        String rhs = decimalPointPosition != -1 ? s.substring(decimalPointPosition + 1) : "";
  //        return new LeftAdjuster(numIntegerDigits, CHARACTER_ZERO).filter(lhs, padChar) + new
  // RightAdjuster(numFractionalDigits, CHARACTER_ZERO).filter(rhs, padChar);
  //    }
