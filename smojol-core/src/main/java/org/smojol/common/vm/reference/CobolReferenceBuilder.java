...
public class CobolReferenceBuilder {
    ...
    private static CobolDataStructure resolve(CobolParser.QualifiedDataNameContext qualifiedDataNameContext, CobolDataStructure data) {
        ...
        List<CobolParser.ArithmeticExpressionContext> indices = qualifiedDataNameContext.tableCall().argument().stream().map(CobolParser.ArgumentContext::arithmeticExpression).toList();
