  ...
  public CobolExpression visitMultDivs(CobolParser.MultDivsContext ctx) {
    ...
    expression =
        ArithmeticExpression.expression(
            firstTerm.MINUSCHAR() != null
                ? UnaryMathOperations.NEGATIVE
                : UnaryMathOperations.POSITIVE,
            powersVisitor.getExpression());
    ...
    for (CobolParser.MultDivContext m : ctx.multDiv()) {
      ...
      expression =
          ArithmeticExpression.expression(
              m.ASTERISKCHAR() != null
                  ? BinaryMathOperations.MULTIPLY
                  : BinaryMathOperations.DIVIDE,
              expression,
              secondTermVisitor.getExpression());
    }
    return expression;
  }
}
