...
public class Comp3DataTypeSpec extends DataTypeSpec {
  ...
  @Getter private final Comp3SignType signType;
  ...
  private String numberAsString(MemoryAccess access) {
    ...
    String fullByteStringWithSign =
        bytes.stream()
            .map(b -> String.format("%02X", b))
            .reduce("", (byteString, b) -> byteString + b);
  ...
  public Object readFormatted(MemoryAccess access) {
    return signType.signed(
        Double.parseDouble(
            new StringBuilder(numberAsString(access)).insert(leftSideDigits + 1, ".").toString()),
        access);
  }
  ...
  public void set(String s, MemoryAccess access) {
    ...
    List<Byte> allBytes =
        recurse(new StringBuilder(remainingDigits).reverse().toString()).reversed();
  ...
  public List<Byte> recurse(String remainingDigits) {
    ...
    Byte currentByte =
        HexMapper.hexStringToByte(remainingDigits.charAt(1), remainingDigits.charAt(0));
