...
public class EvaluateBreaker {
  ...
  public EvaluateBreaker(
      StackFrames staticFrameContext, FlowNode parent, FlowNodeService nodeService) {
  ...
  public ExpandedEvaluation decompose(CobolParser.EvaluateStatementContext whenStatement) {
    List<FlowNode> elseBody =
        whenStatement.evaluateWhenOther() != null
            ? whenStatement.evaluateWhenOther().conditionalStatementCall().stream()
                .map(stmt -> nodeService.node(stmt, parent, this.staticFrameContext))
                .toList()
            : ImmutableList.of();
    evaluationSubjects.add(selectionSubject(whenStatement.evaluateSelect(), dataStructures));
    evaluationSubjects.addAll(
        whenStatement.evaluateAlsoSelect().stream()
            .map(subj -> selectionSubject(subj.evaluateSelect(), dataStructures))
            .toList());
    List<TestActionPair> whenPhraseFlowNodes =
        whenStatement.evaluateWhenPhrase().stream().map(this::conditionGroup).toList();
    return new ExpandedEvaluation(evaluationSubjects, whenPhraseFlowNodes, elseBody);
  }

  public CobolExpression selectionSubject(
      CobolParser.EvaluateSelectContext selectionSubject, CobolDataStructure dataStructures) {
  ...
  public TestActionPair conditionGroup(CobolParser.EvaluateWhenPhraseContext whenPhrase) {
    CobolExpression condition =
        recursiveOr(head(whenPhrase.evaluateWhen()), tail(whenPhrase.evaluateWhen()));
    List<FlowNode> body =
        whenPhrase.conditionalStatementCall().stream()
            .map(stmt -> nodeService.node(stmt, parent, this.staticFrameContext))
            .toList();
    return new TestActionPair(condition, body);
  }

  private CobolExpression recursiveOr(
      Optional<CobolParser.EvaluateWhenContext> head,
      List<CobolParser.EvaluateWhenContext> remaining) {
    if (remaining.isEmpty()) return mustHaveConditions(head.get());
    return new OrExpression(
        mustHaveConditions(head.get()), recursiveOr(head(remaining), tail(remaining)));
  }
  ...
  private CobolExpression mustHaveConditions(CobolParser.EvaluateWhenContext ctx) {
    ...
    conditions.add(
        condition(ctx.evaluateCondition(), evaluationSubjects.getFirst(), dataStructures));
    List<ImmutablePair<CobolParser.EvaluateAlsoConditionContext, CobolExpression>>
        subjectConditionPairs =
            zip(
                    ctx.evaluateAlsoCondition().stream(),
                    tail(evaluationSubjects).stream(),
                    (a, b) -> ImmutablePair.of(a, b))
                .toList();
    conditions.addAll(
        subjectConditionPairs.stream()
            .map(p -> condition(p.getLeft().evaluateCondition(), p.getRight(), dataStructures))
            .toList());
    return recursiveAnd(head(conditions), tail(conditions));
  }

  private CobolExpression recursiveAnd(
      Optional<CobolExpression> head, List<CobolExpression> remaining) {
  ...
  private CobolExpression condition(
      ...
      CobolExpression associatedSubject,
      ...
      return new SimpleConditionExpression(
          ...
          new RelationExpression(
              ...
              new CobolExpressionBuilder()
                  .condition(evaluateConditionContext.condition(), dataStructures)));
    else if (evaluateConditionContext.booleanLiteral() != null)
      return new SimpleConditionExpression(
          associatedSubject,
          new PrimitiveCobolExpression(
              TypedRecord.typedBoolean(
                  Boolean.parseBoolean(evaluateConditionContext.booleanLiteral().getText()))));
    else if (evaluateConditionContext.ANY() != null)
      return new SimpleConditionExpression(
          associatedSubject, new PrimitiveCobolExpression(TypedRecord.TRUE));
    CobolExpression fromExpression =
        cobolExpressionBuilder.arithmetic(
            evaluateConditionContext.evaluateValue().arithmeticExpression());
    List<CobolExpression> argRange =
        ImmutableList.of(
            associatedSubject,
            fromExpression,
    ...
    return evaluateConditionContext.NOT() != null
        ? new NotExpression(isInRangeCall)
        : isInRangeCall;
  }

  private CobolExpression throughExpression(
      CobolParser.EvaluateConditionContext evaluateConditionContext,
      CobolExpression defaultRhsExpression) {
    if (evaluateConditionContext.evaluateThrough() == null) return defaultRhsExpression;
    return cobolExpressionBuilder.arithmetic(
        evaluateConditionContext.evaluateThrough().evaluateValue().arithmeticExpression());
  }
}
