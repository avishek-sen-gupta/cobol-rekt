...
public class ExecutionInterceptors implements ExecutionInterceptor {
  ...
  private Supplier<CobolVmSignal> runRecursively(
      ListIterator<ExecutionInterceptor> iterator,
      Supplier<CobolVmSignal> execution,
      ExecutionContext executionContext) {
    if (!iterator.hasNext()) return execution;
    ExecutionInterceptor interceptor = iterator.next();
    return () ->
        interceptor.run(runRecursively(iterator, execution, executionContext), executionContext);
  }
