...
public class ZonedDecimalDataTypeSpec extends DataTypeSpec {
  ...
  public ZonedDecimalDataTypeSpec(
      int leftSideDigits, int rightSideDigits, ZonedDecimalSignType signType) {
  ...
  private String numberAsString(MemoryAccess access) {
    ...
    return digits(region).stream()
        .map(b -> Byte.toString((byte) (b & 0x0F)))
        .reduce("", (a, b) -> a + b);
  }
  ...
  public Object readFormatted(MemoryAccess access) {
    return signType.signed(
        Double.parseDouble(
            new StringBuilder(numberAsString(access)).insert(leftSideDigits, ".").toString()),
        access);
  }
  ...
  public void refresh(MemoryAccess access) {
    ...
    boolean isPositive =
        signType == ZonedDecimalSignType.UNSIGNED
            || (signType == ZonedDecimalSignType.SIGNED
                && ((region.asBytes().getLast() & 0xF0) != 0xD0));

    String overflowingNumber =
        digits.stream().map(HexMapper::hexChar).reduce((a, b) -> a + b).get();
