...
public class ConditionVisitor extends AntlrCobolExpressionVisitor {
  ...
  public CobolExpression visitCondition(CobolParser.ConditionContext ctx) {
    if (ctx.simpleCondition() != null) {
      SimpleConditionVisitor simpleConditionVisitor =
          new SimpleConditionVisitor(null, null, dataRoot);
    ...
    ComparisonOperator mostRecentOperator =
        expression.getClass() == SimpleConditionExpression.class
            ? ((SimpleConditionExpression) expression).getComparison().getRelationalOperation()
            : null;
    CobolExpression mostRecentLhs =
        expression.getClass() == SimpleConditionExpression.class
            ? ((SimpleConditionExpression) expression).getLhs()
            : null;
    for (CobolParser.AdditionalConditionContext c : ctx.additionalCondition()) {
      AdditionalConditionVisitor additionalConditionVisitor =
          new AdditionalConditionVisitor(mostRecentLhs, mostRecentOperator, dataRoot);
