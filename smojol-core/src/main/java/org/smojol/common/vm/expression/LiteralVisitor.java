...
public class LiteralVisitor extends AntlrCobolExpressionVisitor {
    ...
    if (ctx.numericLiteral() != null) {
      expression =
          new PrimitiveCobolExpression(
              TypedRecord.typedNumber(asNumber(ctx.numericLiteral().getText())));
    } else if (ctx.booleanLiteral() != null) {
      expression =
          new PrimitiveCobolExpression(
              TypedRecord.typedBoolean(asBoolean(ctx.booleanLiteral().getText())));
    } else if (ctx.charString() != null) {
      expression =
          new PrimitiveCobolExpression(TypedRecord.typedString(ctx.numericLiteral().getText()));
    } else if (ctx.figurativeConstant() != null) {
      expression =
          new PrimitiveCobolExpression(
              typed(new FigurativeConstantMap().map(ctx.figurativeConstant().getText())));
    } else if (ctx.NONNUMERICLITERAL() != null) {
      expression =
          new PrimitiveCobolExpression(
              TypedRecord.typedString(
                  ConversionStrategy.asString(ctx.NONNUMERICLITERAL().getText())));
    }
    return expression;
  }
  ...
  private TypedRecord typed(String constant) {
    try {
      ...
      // TODO: This is a hack; find the correct context that will decide what type the literal
      // should be unambiguously converted
      LOGGER.warning(
          ConsoleColors.red(
              "Could not convert '"
                  + constant
                  + "' to a number, returning the original string..."));
