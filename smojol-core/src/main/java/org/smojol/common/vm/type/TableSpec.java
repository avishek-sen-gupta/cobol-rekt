...
public class TableSpec extends DataTypeSpec {
  ...
  public MemoryLayout index(int index, MemoryAccess access) {
    if (index < 0 || index >= coreType.sizeInBytes())
      throw new IllegalIndexException(index, "Index out of bounds");
    MemoryRegion region = access.get();
    RangeMemoryAccess subscriptAccess =
        new RangeMemoryAccess(
            region,
            index * coreType.sizeInBytes(),
            index * coreType.sizeInBytes() + coreType.sizeInBytes() - 1);
