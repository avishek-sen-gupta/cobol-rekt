...
public enum ZonedDecimalSignType {
  ...
  SIGNED {
    ...
    public Double signedNumber(double v, MemoryAccess access) {
      ...
      logger.warning(
          ConsoleColors.coloured(
              "Unknown sign bit in signed type, possible bug in set() of ZonedDecimalType",
              0,
              202));
      return v;
    }
  ...
  public Double signed(double v, MemoryAccess access) {
    if (v < 0) {
      ...
      logger.warning(
          ConsoleColors.red(
              "Value is already negative. Potential misrepresentation of memory region!"));
      return v;
    }
