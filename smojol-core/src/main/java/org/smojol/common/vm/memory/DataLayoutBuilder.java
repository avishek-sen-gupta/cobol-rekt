...
public class DataLayoutBuilder {
  ...
  private Pair<DataTypeSpec, Integer> typeSpec(
      CobolDataTypes.DataTypeSpecContext dataTypeSpecContext) {
  ...
  private ImmutablePair<DataTypeSpec, Integer> numeric(CobolDataTypes.DataTypeSpecContext spec) {
    ...
    ZonedDecimalSignType signType =
        fractionCtx.SIGN_SYMBOL() != null
            ? ZonedDecimalSignType.SIGNED
            : ZonedDecimalSignType.UNSIGNED;
    Pair<Integer, Integer> leftRight = numChars(fractionCtx);
    ZonedDecimalDataTypeSpec numericType =
        new ZonedDecimalDataTypeSpec(leftRight.getLeft(), leftRight.getRight(), signType);
    int totalChars = leftRight.getRight() + leftRight.getLeft();
    return new ImmutablePair<>(numericType, totalChars);
  }

  private ImmutablePair<DataTypeSpec, Integer> alphanumericLayout(
      CobolDataTypes.DataTypeSpecContext spec) {
    CobolDataTypes.AlphanumericContext alphanumeric = spec.alphanumeric();
    List<CobolDataTypes.LeftSideAlphanumericIndicatorContext> leftSide =
        alphanumeric.leftSideAlphanumericIndicator();
    List<CobolDataTypes.RightSideAlphanumericIndicatorContext> rightSide =
        alphanumeric.rightSideAlphanumericIndicator();
  ...
  private Pair<Integer, Integer> numChars(CobolDataTypes.FractionContext fractionCtx) {
    if (fractionCtx.integer() != null) {
      ...
      List<CobolDataTypes.DigitIndicatorContext> indicators =
          fractionCtx.onlyLeftOfDecimalPoint().integerPart().digitIndicator();
      Integer leftSide = numChars(indicators);
      Integer rightSide = 0;
      return new ImmutablePair<>(leftSide, rightSide);
    } else if (fractionCtx.onlyRightOfDecimalPoint() != null) {
      List<CobolDataTypes.DigitIndicatorContext> indicators =
          fractionCtx.onlyRightOfDecimalPoint().fractionalPart().digitIndicator();
      Integer leftSide = 0;
      Integer rightSide = numChars(indicators);
      return new ImmutablePair<>(leftSide, rightSide);
    } else {
      Integer leftSide =
          numChars(fractionCtx.bothSidesOfDecimalPoint().integerPart().digitIndicator());
      Integer rightSide =
          numChars(fractionCtx.bothSidesOfDecimalPoint().fractionalPart().digitIndicator());
      return new ImmutablePair<>(leftSide, rightSide);
    }
  }
  ...
  private int numChars(CobolDataTypes.RightSideAlphanumericIndicatorContext indicator) {
    if (indicator.alphaNumericIndicator() != null)
      return numChars(indicator.alphaNumericIndicator());
  ...
  private int numChars(CobolDataTypes.DigitIndicatorContext digitIndicator) {
    return digitIndicator.numberTypeIndicator() != null
        ? 1
        : Integer.parseInt(
            digitIndicator.ndigits().numberOf().getText().replace("(", "").replace(")", ""));
  }

  private int numChars(CobolDataTypes.AlphaNumericIndicatorContext alphanumericIndicator) {
    return alphanumericIndicator.charTypeIndicator() != null
        ? 1
        : Integer.parseInt(
            alphanumericIndicator.nchars().numberOf().getText().replace("(", "").replace(")", ""));
  }
