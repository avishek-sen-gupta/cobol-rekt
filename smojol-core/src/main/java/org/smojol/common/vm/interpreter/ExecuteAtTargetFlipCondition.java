...
/**
 * This is specifically used to skip over statements when navigating to a GO TO target which is in
 * the middle of a section, while still building the correct stack frames. Nodes which are
 * encountered before the GO TO target are simply not executed. When the target is encountered, the
 * internal switch is flipped to true, and stays true thereafter.
 */
public class ExecuteAtTargetFlipCondition implements ExecuteCondition {
  private static final java.util.logging.Logger LOGGER =
      Logger.getLogger(ExecuteAtTargetFlipCondition.class.getName());
