...
public class SimpleConditionVisitor extends AntlrCobolExpressionVisitor {
  ...
  @Setter private CobolExpression comparison;

  public SimpleConditionVisitor(
      CobolExpression mostRecentLhs,
      ComparisonOperator mostRecentRelationalOperation,
  ...
  public CobolExpression visitSimpleCondition(CobolParser.SimpleConditionContext ctx) {
    ...
    if (lhs instanceof VariableExpression varExpr) {
      CobolReference reference =
          new CobolReferenceBuilder().getReference(varExpr.getName(), dataRoot);
      CobolDataStructure resolved = reference.resolve();
      if (resolved.getClass() == NullDataStructure.class) {
        // TODO: Temp fix for variables which don't directly appear in data structures like indexes
        // in INDEXED BY clauses
        ...
        expression =
            new SimpleConditionExpression(
                new VariableExpression(cds.parent().name()),
                new RelationExpression(
                    RelationalOperation.EQUAL, new VariableExpression(cds.name())));
        expression =
            new FunctionCallExpression(
                "isInRange",
                ImmutableList.of(
                    new VariableExpression(cds.parent().name()),
                    new VariableExpression(cds.name())));
        return expression;
        //                cds.parent().
        //                expression = lhs;
        //                return expression;
