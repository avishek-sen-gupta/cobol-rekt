...
public class ExpressionComparisonVisitor extends AntlrCobolExpressionVisitor {
  @Override
  public CobolExpression visitRelationCombinedComparison(
      CobolParser.RelationCombinedComparisonContext ctx) {
    ComparisonOperator relationalOperation = RelationalOperations.create(ctx.relationalOperator());
    if (ctx.arithmeticExpression() != null) {
      expression =
          new RelationExpression(
              relationalOperation,
              new CobolExpressionBuilder().arithmetic(ctx.arithmeticExpression()));
    } else if (ctx.multipleArithmeticExpressions() != null) {
      CobolParser.ArithmeticExpressionContext firstTerm =
          ctx.multipleArithmeticExpressions().arithmeticExpression();
      expression = new CobolExpressionBuilder().arithmetic(firstTerm);

      List<CobolParser.AdditionalArithmeticExpressionContext> additionalExpressions =
          ctx.multipleArithmeticExpressions().additionalArithmeticExpression();
      ...
      for (CobolParser.AdditionalArithmeticExpressionContext c : additionalExpressions) {
        ...
        CobolExpression arithmeticExpression =
            new CobolExpressionBuilder().arithmetic(additionalTerm);
