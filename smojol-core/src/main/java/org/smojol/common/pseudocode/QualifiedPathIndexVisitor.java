...
/**
 * Scoped visitor that builds a bare-name -> [(fullPath, SymbolReference)] index. Each child
 * invocation returns a new visitor with the extended path. The index map is shared across all
 * visitors (accumulated during traversal). Reuses SymbolReference objects from the already-built
 * bare-name symbols map to avoid creating duplicate SymbolReference instances for the same node.
 */
