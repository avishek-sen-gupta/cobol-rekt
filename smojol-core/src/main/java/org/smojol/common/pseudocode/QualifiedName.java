...
public record QualifiedName(List<String> parts) {
  ...
  public static QualifiedName of(String bareName, List<String> qualifiers) {
    return new QualifiedName(Stream.concat(Stream.of(bareName), qualifiers.stream()).toList());
  }
   ...
   *
   * <p>{@code candidate} = full root-to-leaf path, outermost-first, e.g. ["ROOT", "STRUCT-1",
   * "FIELD-A"]. {@code parts} = [bareName, qualifier1, ...] innermost-first.
   *
   * <p>The check: reverse parts to outermost-first order, then verify they form a (non-contiguous)
   * subsequence that ends at the tail of candidate. The last element of the reversed query must
   * match the last element of candidate.
   */
