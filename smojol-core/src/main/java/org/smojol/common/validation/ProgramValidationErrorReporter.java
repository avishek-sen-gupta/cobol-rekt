...
public class ProgramValidationErrorReporter {
  private static final Logger LOGGER =
      Logger.getLogger(ProgramValidationErrorReporter.class.getName());
  ...
  private void reportProgram(ProgramValidationErrors ve) {
    ...
    LOGGER.info(
        String.format(
            "Validation Summary: Program %s%n--------------------------------------------------------------%n",
            ve.getProgramFileName()));
  ...
  private void reportReferenceErrors(ProgramValidationErrors errors) {
    LOGGER.info(
        String.format(
            "Reference Errors: Program %s%n--------------------------------------------------------------%n",
            errors.getProgramFileName()));
    if (!errors.hasReferenceErrors()) {
      LOGGER.info(
          ConsoleColors.green(
              String.format("No Syntax Errors for %s", errors.getProgramFileName())));
      return;
    }
    String undefinedStructuresList = String.join(",", errors.getReferenceErrors());
    LOGGER.info(
        String.format(
            "The following data structures referenced in the program were not defined: %s",
            undefinedStructuresList));
  }

  private void reportSyntaxErrors(
      ProgramValidationErrors singleProgramValidationErrors, Function<SyntaxError, String> format) {
    LOGGER.info(
        String.format(
            "Validation Details: Program %s%n--------------------------------------------------------------%n",
            singleProgramValidationErrors.getProgramFileName()));
    if (!singleProgramValidationErrors.hasSyntaxErrors()) {
      LOGGER.info(
          ConsoleColors.green(
              String.format(
                  "No Syntax Errors for %s", singleProgramValidationErrors.getProgramFileName())));
      return;
    }
    singleProgramValidationErrors
        .getSyntaxErrors()
        .forEach(e -> LOGGER.info(String.format("%s", format.apply(e))));
  }
  ...
  private void reportNonSyntaxErrors(ProgramValidationErrors singleProgramValidationErrors) {
    ...
    for (String e : singleProgramValidationErrors.getNonSyntaxErrors()) {
      LOGGER.info(
          String.format(
              "Non-Syntax Error for %s: %s",
              singleProgramValidationErrors.getProgramFileName(), ConsoleColors.red(e)));
    }
  }
}
