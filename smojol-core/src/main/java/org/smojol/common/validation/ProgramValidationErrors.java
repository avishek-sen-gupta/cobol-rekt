...
public class ProgramValidationErrors {
  ...
  public ProgramValidationErrors(
      String programFileName,
      List<SyntaxError> syntaxErrors,
      List<String> referenceErrors,
      ImmutableList<String> nonSyntaxErrors) {
  ...
  private ProgramValidationErrors(String programFilename, ParseTree[] unreferencedIdentifiers) {
    this(
        programFilename,
        ImmutableList.of(),
        Arrays.stream(unreferencedIdentifiers).map(ParseTree::getText).distinct().toList(),
        ImmutableList.of());
  }
  ...
  public static ProgramValidationErrors usageErrors(
      String programFilename, List<ParseTree> unreferencedIdentifiers) {
    return new ProgramValidationErrors(
        programFilename, unreferencedIdentifiers.toArray(new ParseTree[0]));
  }

  public static ProgramValidationErrors parseErrors(
      String programFilename, List<SyntaxError> errors) {
