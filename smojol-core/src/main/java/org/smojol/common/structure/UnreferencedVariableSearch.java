...
public class UnreferencedVariableSearch {
  public List<ParseTree> run(CobolEntityNavigator navigator, CobolDataStructure dataStructures) {
    List<ParseTree> allVariableUsages =
        navigator.findAllByCondition(
            n -> n.getClass() == CobolParser.VariableUsageNameContext.class);
    return allVariableUsages.stream()
        .filter(
            gid -> dataStructures.reference(gid.getText()).getClass() == NullDataStructure.class)
        .distinct()
        .toList();
  }
}
