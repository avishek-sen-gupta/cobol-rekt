...
public class DataTypesTest {
  ...
  /**
   * This is essentially an invalid move, and the result is undefined. This reference clearly
   * identifies this as invalid:
   * https://www.ibm.com/docs/en/cobol-zos/6.3?topic=moves-valid-invalid-elementary However, the
   * z/OS compiler seems to interpret this correctly as -060, +060 internally. This test is here to
   * document that Smojol does not treat this specially, and thus, gives results different from
   * z/OS.
   */
  @Test
  public void
      doesNotHandleZOSSpecialCaseWhenInterpreterDoesNotIgnoreSignIfImpliedDecimalPointIsPresentAndRawDataHasNegative() {
  ...
  public void cannotIndexNonTableMemory() {
    ...
    IllegalIndexException nonTableSpecException1 =
        assertThrows(IllegalIndexException.class, () -> numericLayout.index(0));
    assertEquals(
        "Illegal indexing attempt: 0. Cause: Non-TableSpec type",
        nonTableSpecException1.getMessage());
    ...
    MemoryLayout alphaNumericLayout =
        new MemoryLayout(alphaNumericMemoryRegion.fullAccess(), alphanumeric);
    IllegalIndexException nonTableSpecException2 =
        assertThrows(IllegalIndexException.class, () -> alphaNumericLayout.index(0));
    assertEquals(
        "Illegal indexing attempt: 0. Cause: Non-TableSpec type",
        nonTableSpecException2.getMessage());
  }
  ...
  public void canErrorOnOutOfBoundsIndexAccess() {
    ...
    assertEquals(15, tableSpec.sizeInBytes());
    MemoryLayout layout = new MemoryLayout(memoryRegion.fullAccess(), tableSpec);
    IllegalIndexException exceptionIndexGreaterThanMaximumSize =
        assertThrows(IllegalIndexException.class, () -> layout.index(6));
    assertEquals(
        "Illegal indexing attempt: 6. Cause: Index out of bounds",
        exceptionIndexGreaterThanMaximumSize.getMessage());
    IllegalIndexException exceptionIndexLessThanZero =
        assertThrows(IllegalIndexException.class, () -> layout.index(-6));
    assertEquals(
        "Illegal indexing attempt: -6. Cause: Index out of bounds",
        exceptionIndexLessThanZero.getMessage());
  }
  ...
  public void canPerformArithmeticOperationsWithUnsignedComp3Data() {
    ...
    MemoryLayout resultLayout =
        new MemoryLayout(resultMemoryRegion.fullAccess(), comp3DataTypeSpec);
    layout1.set("12.34");
    layout2.set("12.34");
