...
/**
 * Unit tests for {@link DialectIntegratorListener} handling of missing keys in {@link
 * PersistentData}.
 *
 * <p>Issue 1 (HIGH): When the IDMS key requested by a {@code dialectNodeFiller} placeholder is
 * absent from {@code PersistentData}, the listener must skip gracefully rather than throwing a
...
class DialectIntegratorListenerMissingKeyTest {
  ...
  /**
   * When {@code PersistentData} has no entry for a given IDMS key, calling {@code
   * enterDialectNodeFiller} must not throw any exception.
   *
   * <p>Before the fix, {@code PersistentData.dialect("IDMS-99")} called {@code
   * Objects.requireNonNull(getDialectNode(...))} which would throw a bare {@code
   * NullPointerException} with no diagnostic message.
   */
  @Test
  ...
  /**
   * A null {@code dialectGuid()} (guard already present) must still be handled without exception,
   * confirming the early-return path for null guid is unaffected by the new fix.
   */
  @Test
