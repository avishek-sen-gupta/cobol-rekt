...
public class TreeSmithTest {
  ...
  public void canEscapeArbitraryScopeUsingJumpIfForForwardJump() {
    ...
    EqualToNode condition =
        new EqualToNode(
            new SymbolReferenceNode("abcd"), new PrimitiveValueTranspilerNode(TypedRecord.TRUE));
    ...
    IfTranspilerNode ifStmt =
        new IfTranspilerNode(
            condition,
            new TranspilerCodeBlockNode(ImmutableList.of(gotoSomeplace, set("abcd", 12))));
    TranspilerCodeBlockNode program =
        new TranspilerCodeBlockNode(ImmutableList.of(ifStmt, set1, set2));
    block_(if_(block_(jmp_(), set_()), any_()), set_(), set_()).verify(program);
    ...
    block_(if_(block_(set_(), if_(block_(set_()), any_())), any_()), jmpIf_(), set_(), set_())
        .verify(program);
  }
  ...
  public void canAssertStructure() {
    ...
    IfTranspilerNode ifStmt =
        new IfTranspilerNode(
            condition,
            new TranspilerCodeBlockNode(ImmutableList.of(gotoSomeplace, set("abcd", 12))));
    TranspilerCodeBlockNode blockNode =
        new TranspilerCodeBlockNode(ImmutableList.of(set1, set2, ifStmt));
    block_(set_(), set_(), if_(block_(jmp_(), set_()), any_())).verify(blockNode);
  }
  ...
  public void canEncloseStatementRangeInNewScopeAutomaticallyGivenSimpleJumpNodeForBackJump() {
    ...
    TranspilerNode set3 = set("PQR", 50);
    TranspilerNode set4 = set("KLM", 70);
    JumpTranspilerNode jumpTranspilerNode =
        new JumpTranspilerNode(new NamedLocationNode("SOME_BLOCK"));
    TranspilerNode jumpBlock =
    ...
    TranspilerNode program =
        new TranspilerCodeBlockNode(ImmutableList.of(jumpBlock, set3, set4, jumpTranspilerNode));
    block_(labelledBlock_("SOME_BLOCK", set_(), set_()), set_(), set_(), jmp_()).verify(program);
    assertTrue(new TreeSmith(program).eliminateBackJump(jumpTranspilerNode));
    block_(loop_(block_(labelledBlock_("SOME_BLOCK", set_(), set_()), set_(), set_(), set_())))
        .verify(program);
  }
  ...
  public void canEncloseStatementRangeInNewScopeAutomaticallyGivenJumpIfNodeForBackJump() {
    ...
    TranspilerNode program =
        new TranspilerCodeBlockNode(ImmutableList.of(jumpBlock, set3, set4, jumpTranspilerNode));
    block_(labelledBlock_("SOME_BLOCK", set_(), set_()), set_(), set_(), jmpIf_()).verify(program);
    assertTrue(new TreeSmith(program).eliminateBackJump(jumpTranspilerNode));
    block_(loop_(block_(labelledBlock_("SOME_BLOCK", set_(), set_()), set_(), set_())))
        .verify(program);
  }
  ...
  public void canEncloseStatementRangeInNewIfScopeAutomaticallyGivenSimpleJumpNodeForForwardJump() {
    ...
    block_(jmp_(), set_(), set_(), set_(), labelledBlock_("SOME_BLOCK", set_(), set_()))
        .verify(program);
    assertTrue(new TreeSmith(program).eliminateForwardJump(jumpTranspilerNode));
    block_(
            set_(),
            if_(block_(set_(), set_(), set_()), any_()),
            labelledBlock_("SOME_BLOCK", set_(), set_()))
        .verify(program);
  }
  ...
  public void canEscapeScopeOnceAutomaticallyToCorrectLevelGivenJumpIfNode() {
    ...
    TranspilerNode program =
        new TranspilerCodeBlockNode(
            ImmutableList.of(ifStmt, set3, set4, set5, jumpDestinationBlock));
    block_(
    ...
    block_(
            if_(
                block_(
                    block_(set_(), if_(block_(set_()), any_())),
                    set_(),
                    if_(block_(set_()), any_())),
                any_()),
            jmpIf_(),
            set_(),
            set_(),
  ...
  public void canEliminateBackwardGotoByCorrectLevelPromotion() {
    ...
    TranspilerNode program =
        new TranspilerCodeBlockNode(
            ImmutableList.of(jumpDestinationBlock, set3, set4, set5, ifStmt));
    block_(
            labelledBlock_("SOME_BLOCK", set_(), set_()),
            set_(),
            set_(),
