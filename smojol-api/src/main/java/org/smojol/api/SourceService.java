...
public class SourceService {
  ...
  public Long insertUnifiedModel(
      SerialisableUnifiedModel model, String programName, long projectID, DSLContext using) {
    return using
        .insertInto(UNIFIED_FLOW)
        .columns(PROGRAM_NAME_FIELD, PROJECT_ID_FIELD, BODY_FIELD)
  ...
  public Optional<Map<String, Object>> flowModel(long id, DSLContext using) {
    @NotNull
    Result<Record2<Long, String>> flowModels =
        using.select(ID_FIELD, BODY_FIELD).from(UNIFIED_FLOW).where(ID_FIELD.eq(id)).fetch();
    if (flowModels.isEmpty()) return Optional.empty();
    return Optional.of(
        ImmutableMap.of(
            "id", id, "body", gson.fromJson(flowModels.getFirst().component2(), JsonObject.class)));
  }

  public Optional<Map<String, Object>> rawAST(long id, DSLContext using) {
    @NotNull
    Result<Record2<Long, String>> rawASTs =
        using.select(ID_FIELD, AST_BODY_FIELD).from(RAW_AST).where(ID_FIELD.eq(id)).fetch();
    if (rawASTs.isEmpty()) return Optional.empty();
    return Optional.of(
        ImmutableMap.of(
            "id", id, "ast", gson.fromJson(rawASTs.getFirst().component2(), JsonObject.class)));
  }

  public Long insertFlowchart(
      ...
      DSLContext using) {
    return using
        .insertInto(FLOWCHART)
        .columns(PROGRAM_NAME_FIELD, PROJECT_ID_FIELD, field("SECTION"), MARKUP_FIELD)
  ...
  public Optional<Map<String, Object>> flowchart(long id, DSLContext using) {
    @NotNull
    Result<Record2<Long, String>> flowcharts =
        using.select(ID_FIELD, MARKUP_FIELD).from(FLOWCHART).where(ID_FIELD.eq(id)).fetch();
  ...
  public long insertRawAST(
    ...
    return using
        .insertInto(RAW_AST)
        .columns(PROGRAM_NAME_FIELD, PROJECT_ID_FIELD, AST_BODY_FIELD)
