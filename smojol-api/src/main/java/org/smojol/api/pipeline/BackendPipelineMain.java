...
public class BackendPipelineMain {
  public static void main(String[] args) throws IOException, InterruptedException, SQLException {
    ...
    Map<String, List<AnalysisTaskResult>> analysisResult =
        new CodeTaskRunner(
            ...
            .runForPrograms(
                ImmutableList.of(
                    BUILD_BASE_ANALYSIS, CommandLineAnalysisTask.BUILD_TRANSPILER_FLOWGRAPH),
    ...
    SerialisableUnifiedModel unifiedModel =
        new UnifiedFlowModelTask(
                baseModel.flowRoot(),
                baseModel.dataStructures(),
                new NodeSpecBuilder(new NamespaceQualifier("TEST_NAMESPACE")))
            .run();
    ...
    Graph<BasicBlock<TranspilerInstruction>, DefaultEdge> blockGraph =
        transpilerFlowgraph.basicBlockFlowgraph();
    Graph<TranspilerInstruction, DefaultEdge> instructionFlowgraph =
        transpilerFlowgraph.instructionFlowgraph();
    ...
    Pair<Set<NaturalLoopBody<TranspilerInstruction>>, Set<NaturalLoopBody<TranspilerInstruction>>>
        loopBodies =
            new LoopBodyDetectionTask<>(
                    transpilerFlowgraph.instructions().getFirst(),
                    transpilerFlowgraph.instructionFlowgraph(),
                    DefaultEdge.class,
                    CloneEdgeOperation::cloneEdge)
                .run();
    ...
    AnalysisTaskResultOK reducibilityTestResult =
        (AnalysisTaskResultOK)
            new T1_T2_IntervalAnalysisTask<>(
                    duplicateGraph(instructionFlowgraph, DefaultEdge.class, e -> new DefaultEdge()),
                    n -> n == transpilerFlowgraph.instructions().getFirst(),
                    (from, to) -> new DefaultEdge())
                .run();
    FlowgraphReductionResult<TranspilerInstruction, DefaultEdge> reductionResult =
        reducibilityTestResult.getDetail();
    System.out.println("Was reducible = " + reductionResult.isReducible());
    reducibleLoopBodies.forEach(
        loop -> {
          System.out.println("-----------------------------");
          System.out.println(
              String.join(",", loop.loopNodes().stream().map(TranspilerInstruction::id).toList()));
          System.out.println("-----------------------------");
    ...
    long projectID =
        insertIntoDB(
            ...
            flowchartMarkup,
            dbContext,
            baseModel.serialisableAST());
    List<JumpTranspilerNode> gotos = tree.allOfType(JumpTranspilerNode.class);
    List<JumpTranspilerNode> demoGotos =
        gotos.stream().filter(g -> g.getStart() instanceof NamedLocationNode).toList();
    TreeSmith treeSmith = new TreeSmith(tree);
    Boolean allEliminated =
        demoGotos.stream().map(treeSmith::eliminateGoto).reduce(true, (a, b) -> a && b);
    System.out.println("Worked: " + allEliminated);
    insertRestructuredAST(tree, programName, projectID, dbContext);
  }

  private static void insertRestructuredAST(
      TranspilerNode tree, String programName, long projectID, DbContext dbContext)
      throws SQLException {
    ...
    dbContext.execute(
        using ->
            intermediateFormService.insertIntermediateAST(
                tree, programName + " [Refactored]", projectID, using));
  }

  private static long insertIntoDB(
      TranspilerFlowgraph transpilerFlowgraph,
      Set<NaturalLoopBody<TranspilerInstruction>> reducibleLoopBodies,
      String programName,
      FlowgraphReductionResult<TranspilerInstruction, DefaultEdge> reductionResult,
      SerialisableUnifiedModel unifiedModel,
      String flowchartMarkup,
      DbContext dbContext,
    ...
    Graph<TranspilerInstruction, DefaultEdge> instructionFlowgraph =
        transpilerFlowgraph.instructionFlowgraph();
    TranspilerNode tree = transpilerFlowgraph.transpilerTree();
    ImmutableMap<String, Set<?>> irCFGForDB =
        ImmutableMap.of(
            "nodes", instructionFlowgraph.vertexSet(), "edges", instructionFlowgraph.edgeSet());
    Gson gson = BuildTranspilerFlowgraphTask.initGson();
    ...
    long pID =
        dbContext.execute(
            using -> {
              ...
              insertIntermediateArtifacts(
                  reducibleLoopBodies,
                  programName,
                  reductionResult,
                  using,
                  intermediateFormService,
                  tree,
                  projectID,
                  irCFGForDB);
  ...
  private static void insertSourceArtifacts(
      SerialisableUnifiedModel unifiedModel,
      String programName,
      long projectID,
      ...
      CobolContextAugmentedTreeNode serialisableAST) {
    long unifiedModelID =
        sourceService.insertUnifiedModel(unifiedModel, programName, projectID, using);
    long flowchartID =
        sourceService.insertFlowchart(
            flowchartMarkup, programName, projectID, "PROCEDURE_DIVISION", using);
    long rawASTID = sourceService.insertRawAST(serialisableAST, programName, projectID, using);
  }

  private static void insertIntermediateArtifacts(
      Set<NaturalLoopBody<TranspilerInstruction>> reducibleLoopBodies,
      String programName,
      FlowgraphReductionResult<TranspilerInstruction, DefaultEdge> reductionResult,
      DSLContext using,
      IntermediateFormService intermediateFormService,
      TranspilerNode tree,
      long projectID,
      ImmutableMap<String, Set<?>> irCFGForDB) {
    long irAstID =
        intermediateFormService.insertIntermediateAST(tree, programName, projectID, using);
    long irCfgID =
        intermediateFormService.insertIntermediateCFG(irCFGForDB, programName, projectID, using);
    List<Long> loopBodyIDs =
        intermediateFormService.insertLoopBody(reducibleLoopBodies, irCfgID, using);
