...
import static org.smojol.ast.MergingChartOverlay.isAtomic;
...
public class FlowNodeOverlayVisitor implements FlowNodeVisitor {
    ...
    public void visit(FlowNode node, List<FlowNode> outgoingNodes, List<FlowNode> incomingNodes, VisitContext context, FlowNodeService nodeService) {
        if ((node.getClass() == SentenceFlowNode.class ||
                ...
                (isAtomic(node) && !contained(node)) // This condition is a little sus because technically, statements inside sentences could also get their own groups which would show up in addition to their parent sentence groups. It's working now, need to investigate with a small test program.
        ) && node.isMergeable()) {
