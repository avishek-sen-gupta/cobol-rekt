  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    Map<String, List<AnalysisTaskResult>> result =
        new CodeTaskRunner(
            ...
            .runForPrograms(
                ImmutableList.of(BUILD_BASE_ANALYSIS),
                ImmutableList.of("reaching-condition-test.cbl"));
    ...
    TranspilerInstruction printInstruction =
        instructions.stream()
            .filter(
                instr ->
                    instr.ref() instanceof PrintTranspilerNode
                        && instr.sentinel() == CodeSentinelType.BODY)
            .findFirst()
            .get();
    ...
    reachingConditions.forEach(
        (key, value1) -> System.out.println(key.description() + ": " + value1.description()));
