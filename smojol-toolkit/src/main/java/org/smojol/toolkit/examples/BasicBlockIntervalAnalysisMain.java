...
public class BasicBlockIntervalAnalysisMain {
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    Function<BasicBlock<TranspilerInstruction>, Boolean> IS_ROOT =
        block -> block == transpilerFlowgraph.basicBlocks().getFirst();
    BiFunction<BasicBlock<TranspilerInstruction>, BasicBlock<TranspilerInstruction>, DefaultEdge>
        DEFAULT_EDGE = (b1, b2) -> new DefaultEdge();
    PruneUnreachableTask.pruneUnreachables(blockGraph, IS_ROOT);
    T1_T2_IntervalAnalysisTask<BasicBlock<TranspilerInstruction>, DefaultEdge> reductionTask =
        new T1_T2_IntervalAnalysisTask<>(blockGraph, IS_ROOT, DEFAULT_EDGE);
    AnalysisTaskResultOK intervalAnalysisResult = (AnalysisTaskResultOK) reductionTask.run();
    FlowgraphReductionResult<BasicBlock<TranspilerInstruction>, DefaultEdge> reductionResult =
        intervalAnalysisResult.getDetail();
