  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    TranspilerFlowgraph transpilerFlowgraph =
        new BuildTranspilerFlowgraphTask(
                baseAnalysisModel.rawAST(),
                baseAnalysisModel.dataStructures(),
                null,
                ImmutableList.of())
            .run();
    System.out.println(
    ...
    String draw =
        new MermaidGraph<TranspilerInstruction, DefaultEdge>()
            .draw(transpilerFlowgraph.instructionFlowgraph());
    PruneUnreachableTask.pruneUnreachableInstructions(transpilerFlowgraph);
    AnalysisTaskResultOK intervalAnalysisResult =
        (AnalysisTaskResultOK)
            new T1_T2_IntervalAnalysisTask<>(
                    transpilerFlowgraph.instructionFlowgraph(),
                    T1_T2_IntervalAnalysisTask.IS_ROOT,
                    T1_T2_IntervalAnalysisTask.NEW_DEFAULT_EDGE)
                .run();
