...
public class ArchitectureMain {
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    List<RawSystemConnectionEntry> allRawConnectionEntries =
        systemEntries.stream()
            .map(ArchitectureMain::systemEntries)
            .reduce(Stream::concat)
            .get()
            .toList();

    List<String> uniqueCurrentSystemNames =
        Stream.concat(
                allRawConnectionEntries.stream().map(RawSystemConnectionEntry::adjacentSystemName),
                allRawConnectionEntries.stream().map(RawSystemConnectionEntry::currentSystemName))
            .distinct()
            .toList();

    Map<String, LegacySystem> uniqueSystems =
        uniqueCurrentSystemNames.stream()
            .map(
                systemName ->
                    new LegacySystem(
                        systemName,
                        allRawConnectionEntries.stream()
                            .filter(sce -> sce.adjacentSystemName().equals(systemName))
                            .map(RawSystemConnectionEntry::systemType)
                            .toList(),
                        cluster(systemName, clusterData)))
            .collect(Collectors.toUnmodifiableMap(LegacySystem::systemName, o -> o));
    List<SystemConnectionEntry> systemConnectionEntries =
        allRawConnectionEntries.stream().map(rce -> systemEntry(rce, uniqueSystems)).toList();
              ...
              sdk.createNode(
                  new WoofNode(
                      ImmutableMap.of(
                          ...
                          system.systemName(),
                          "technologies",
                          techsAsStrings(system),
                          "cluster",
                          system.clusterName()),
                      Stream.concat(
                              Stream.of("SYSTEM", system.clusterName()),
                              techsAsStrings(system).stream())
                          .toList()));
            });
    systemConnectionEntries.forEach(
        sce -> {
          Record systemNode =
              sdk.findNodes(
                      new NodeSpec(
                          ImmutableList.of(),
                          ImmutableMap.of("name", sce.currentSystem().systemName())))
                  .getFirst();
          Record adjacentSystemNode =
              sdk.findNodes(
                      new NodeSpec(
                          ImmutableList.of(),
                          ImmutableMap.of("name", sce.adjacentSystem().systemName())))
                  .getFirst();
          if (sce.connectionDirection() == ConnectionDirection.OUT)
            sdk.connect(systemNode, adjacentSystemNode, "ACCESSES", "SYSTEM_CONNECTION");
          else sdk.connect(adjacentSystemNode, systemNode, "ACCESSES", "SYSTEM_CONNECTION");
        });
    //        System.out.println("Unique currentSystemNames: " + uniqueSystems.keySet());

    List<String> clusterAppNames =
        clusterData.entrySet().stream()
            .filter(e -> e.getValue().equals("AFAB"))
            .map(x -> x.getKey())
            .toList();
    List<LegacySystem> clusterSystems =
        uniqueSystems.values().stream()
            .filter(sys -> clusterAppNames.contains(sys.systemName()))
            .toList();

    List<Pair<LegacySystem, List<Pair<String, List<LegacySystem>>>>>
        clusterPartitionedDependenciesByApp =
            clusterSystems.stream()
                .map(
                    system -> {
                      Record systemNode =
                          sdk.findNodes(
                                  new NodeSpec(
                                      ImmutableList.of(),
                                      ImmutableMap.of("name", system.systemName())))
                              .getFirst();
                      //            System.out.println("ANALYSING SYSTEM: " + system.systemName());
                      List<LegacySystem> unclassifiedChildren =
                          sdk.directChildrenAnyDirection(systemNode, "ACCESSES").stream()
                              .map(
                                  adjacentSystem -> {
                                    //                        System.out.println("Looking for: " +
                                    // adjacentSystem.get("n").get("name"));
                                    LegacySystem foundSystem =
                                        uniqueSystems.get(
                                            adjacentSystem.get("n").get("name").asString());
                                    //                        if (foundSystem == null)
                                    // System.out.println("Could not find system: " +
                                    // adjacentSystem.get("n").get("name"));
                                    return foundSystem;
                                  })
                              .toList();
                      //            unclassifiedChildren.forEach(uc ->
                      // System.out.println("UNCLASSIFIED SYSTEM: " + uc.systemName()));
                      List<Pair<String, List<LegacySystem>>> x =
                          clusterData.values().stream()
                              .distinct()
                              .map(
                                  clusterName ->
                                      Pair.of(
                                          clusterName,
                                          unclassifiedChildren.stream()
                                              .filter(c -> clusterName.equals(c.clusterName()))
                                              .toList()))
                              .toList();
    ...
    clusterPartitionedDependenciesByApp.forEach(
        p -> {
              ...
              .forEach(
                  partition -> {
                    ...
                    partition
                        ...
                        .forEach(
                            partitionedSystem ->
                                System.out.println(partitionedSystem.systemName()));
                    if (partition.getRight().isEmpty())
                      System.out.println("No apps in this partition");
                    System.out.println("--------------------------");
                  });
          System.out.println("===============================");
        });
  ...
  private static SystemConnectionEntry systemEntry(
      RawSystemConnectionEntry rce, Map<String, LegacySystem> uniqueSystems) {
    ...
    return new SystemConnectionEntry(
        currentSystem,
        adjacentSystem,
        rce.connectionDirection(),
        rce.connectionNature(),
        rce.latency(),
        rce.usagePattern());
  }
  ...
  private static ConnectionNature connectionNature(String connectionNature) {
    return "asynchron".equals(connectionNature)
        ? ConnectionNature.ASYNCHRONOUS
        : ConnectionNature.SYNCHRONOUS;
  }
}
