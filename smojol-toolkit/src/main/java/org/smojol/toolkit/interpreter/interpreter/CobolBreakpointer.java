...
public class CobolBreakpointer implements Breakpointer {
  ...
  public CobolVmSignal run(Supplier<CobolVmSignal> execution, ExecutionContext executionContext) {
    ...
    LOGGER.info(
        coloured(
            String.format(
                "Breakpoint hit at %s. Type 'c' to resume, 'q' to halt, 'stack' to print stack. Type any other string to evaluate a variable matching that adjacentSystem.",
                node.originalText().trim()),
            0,
            202));
