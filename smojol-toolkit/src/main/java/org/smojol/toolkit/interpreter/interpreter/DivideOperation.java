...
public class DivideOperation implements CobolOperation {
  ...
  public void run(CobolDataStructure cobolDataStructure) {
    ...
    List<CobolExpression> quotients =
        divide.getDividendExpressions().stream()
            .map(
                dividend ->
                    dividend
                        .evaluate(cobolDataStructure)
                        .divide(divide.getDivisorExpression(), cobolDataStructure))
            .toList();
    ...
    if (quotients.size() == 1)
      destinations.forEach(
          dst ->
              builder
                  .getReference(dst, cobolDataStructure)
                  .set(builder.getReference(quotients.getFirst(), cobolDataStructure)));
    else {
