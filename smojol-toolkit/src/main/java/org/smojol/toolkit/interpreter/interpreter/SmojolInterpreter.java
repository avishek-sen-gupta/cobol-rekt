...
public class SmojolInterpreter implements CobolInterpreter {
  ...
  public SmojolInterpreter(
      StackFrames runtimeStackFrames,
      ExecuteCondition condition,
      ConditionResolver conditionResolver,
      Breakpointer bp,
      List<ExecutionInterceptor> otherInterceptors,
      ExecutionListener listeners,
      CobolOperations operations) {
  ...
  public CobolInterpreter scope(FlowNode scope) {
    return new SmojolInterpreter(
        runtimeStackFrames.add(scope),
        condition,
        conditionResolver,
        breakpointer,
        otherInterceptors,
  ...
  public CobolVmSignal execute(FlowNode node, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          ...
          listeners.notify(
              coloured("Executing " + node.getClass().getSimpleName() + node.label(), 25),
              node,
              nodeService);
  ...
  public void enter(FlowNode node, FlowNodeService nodeService) {
    ...
    String enteringMessage =
        "Entering " + node.getClass().getSimpleName() + "/" + formatted(node.label());
  ...
  public void exit(FlowNode node, FlowNodeService nodeService) {
    listeners.notify(
        coloured("Exiting " + node.getClass().getSimpleName() + "/" + formatted(node.label()), 240),
        node,
        nodeService);
  }
  ...
  public CobolVmSignal executeIf(FlowNode node, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          ...
          if (trueOrFalse) {
            listeners.notify(
                green(ifNode.getCondition().getText() + " was resolved to TRUE"),
                node,
                nodeService);
            ...
            listeners.notify(
                green(ifNode.getCondition().getText() + " was resolved to FALSE"),
                node,
                nodeService);
          ...
          listeners.notify(
              green(
                  ifNode.getCondition().getText()
                      + " was resolved to FALSE, but no ELSE clause was found"),
              node,
              nodeService);
  ...
  @Override
  public CobolVmSignal executePerformProcedure(
      FlowNode node, List<FlowNode> procedures, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          listeners.visit(node, nodeService);
          for (FlowNode procedure : procedures) {
            listeners.notify(
                cyan("Executing a PERFORM statement: " + procedures.getFirst()), node, nodeService);
            CobolVmSignal signal = procedure.acceptInterpreter(this, FlowControl::STOP);
            listeners.notify(
                cyan("Returned from PERFORM statement: " + procedures.getFirst()),
                node,
                nodeService);
          ...
          // If a PERFORM has returned (early or normal termination), do not propagate termination
          // any higher
          return CobolVmSignal.CONTINUE;
        },
        new ExecutionContext(node, runtimeStackFrames, nodeService));
  ...
  @Override
  public CobolVmSignal executeGoto(
      FlowNode node, List<FlowNode> destinationNodes, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          listeners.visit(node, nodeService);
          listeners.notify(
              red("Executing a GOTO statement: " + destinationNodes.getFirst()), node, nodeService);
          FlowNode destination = destinationNodes.getFirst();
          FlowNode continuationNode = actualDestination(destination);
          CobolVmSignal signal =
              continuationNode.acceptInterpreter(locator(destination), FlowControl::CONTINUE);
  ...
  public CobolVmSignal executeExit(FlowNode node, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          ...
          listeners.notify(
              "EXIT instruction is " + coloured(signal.name(), 207), node, nodeService);
          return signal;
        },
        new ExecutionContext(node, runtimeStackFrames, nodeService));
  }
  ...
  @Override
  public CobolVmSignal executeDisplay(
      FlowNode node,
      List<CobolParser.DisplayOperandContext> messages,
      FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          listeners.visit(node, nodeService);
          messages.forEach(
              m -> listeners.notify(coloured("CONSOLE >> " + m.getText(), 154), node, nodeService));
          return CobolVmSignal.CONTINUE;
        },
        new ExecutionContext(node, runtimeStackFrames, nodeService));
  }
                  ...
                  to ->
                      listeners.notify(
                          coloured(
                              String.format(
                                  "%s was affected by %s",
                                  dataDescription(to, nodeService.getDataStructures()),
                                  move.getFromExpressions()),
                              227),
                          node,
                          nodeService));
                          ...
                          purple(
                              coloured(
                                  String.format(
                                      "%s was affected by %s",
                                      dataDescription(
                                          to.generalIdentifier(), nodeService.getDataStructures()),
                                      delimited(add.getFroms())),
                                  227)),
                          node,
                          nodeService));
  ...
  public CobolVmSignal executeSubtract(FlowNode node, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          ...
          listeners.notify(
              purple(coloured(String.format("%s was affected by %s", lhses, rhses), 227)),
              node,
              nodeService);
          ...
          listeners.notify(
              purple(
                  coloured(
                      String.format(
                          "%s was affected by %s",
                          multiply.getLhs(), delimited(multiply.getRhses())),
                      227)),
              node,
              nodeService);
          ...
          listeners.notify(
              purple(
                  coloured(
                      String.format(
                          "%s was affected by %s",
                          delimitedExpressions(divide.getDividendExpressions()),
                          divide.getIntoDivisor()),
                      227)),
              node,
              nodeService);
  ...
  @Override
  public CobolVmSignal executeSearch(
      FlowNode atEndBlock, List<FlowNode> whenPhrases, FlowNodeService nodeService, FlowNode node) {
    return interceptors.run(
        () -> {
          listeners.visit(node, nodeService);
          SearchFlowNode searchNode = (SearchFlowNode) node;
          listeners.notify(
              "Executing SEARCH statement: " + searchNode.getSearchTerm().getText(),
              node,
              nodeService);
          ...
          for (FlowNode whenPhrase : whenPhrases) {
            ...
            listeners.notify(
                "Executing WHEN clause: " + formatted(whenPhrase.label()), node, nodeService);
  ...
  public CobolVmSignal executeOnClause(FlowNode node, FlowNodeService nodeService) {
    return interceptors.run(
        () -> {
          ...
          listeners.notify(
              "Executing ON clause " + formatted(onClauseNode.getCondition().originalText()),
              node,
              nodeService);
          ...
          return onClauseNode
              .getOnClauseBlock()
              .acceptInterpreterForCompositeExecution(this, FlowControl::CONTINUE);
        },
        new ExecutionContext(node, runtimeStackFrames, nodeService));
  }
  ...
  private String dataDescription(
      CobolParser.GeneralIdentifierContext identifier, CobolDataStructure dataStructures) {
  ...
  private CobolInterpreter locator(FlowNode specificLocation) {
    return CobolInterpreterFactory.interpreter(
        new ExecuteAtTargetFlipCondition(specificLocation),
        conditionResolver,
        otherInterceptors,
        listeners,
        breakpointer,
