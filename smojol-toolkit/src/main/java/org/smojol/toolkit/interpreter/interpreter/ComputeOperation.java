...
public class ComputeOperation implements CobolOperation {
  ...
  public void run(CobolDataStructure cobolDataStructure) {
    CobolExpression arithmeticExpression =
        new CobolExpressionBuilder().arithmetic(compute.getRhs());
    PrimitiveCobolExpression value =
        (PrimitiveCobolExpression) arithmeticExpression.evaluate(cobolDataStructure);
    CobolReferenceBuilder referenceBuilder = new CobolReferenceBuilder();
    CobolReference rhs = referenceBuilder.getReference(value);
    //        compute.getDestinations().forEach(d ->
    // cobolDataStructure.set(d.generalIdentifier().getText(), new
    // PrimitiveReference(value.data())));
    compute
        .getDestinations()
        .forEach(
            d ->
                referenceBuilder
                    .getReference(d.generalIdentifier(), cobolDataStructure)
                    .resolve()
                    .set(rhs));
  }
}
