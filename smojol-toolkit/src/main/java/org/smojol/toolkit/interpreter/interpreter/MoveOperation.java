...
public class MoveOperation implements CobolOperation {
  ...
  public void run(CobolDataStructure cobolDataStructure) {
    ...
    List<CobolReference> tos =
        move.getToExpressions().stream()
            .map(to -> referenceBuilder.getReference(to, cobolDataStructure))
            .toList();
    CobolReference from =
        referenceBuilder.getReference(move.getFromExpressions().getFirst(), cobolDataStructure);
    //        CobolExpression from = referenceBuilder.getReference(move.getFromExpression(),
    // cobolDataStructure);
    tos.forEach(to -> to.set(from));
    //        CobolReference fromReference = referenceBuilder.getReference(move.getFrom(),
    // cobolDataStructure);

    //        move.getTos().forEach(to -> cobolDataStructure.set(to.getText(), fromReference));
    //        move.getTos().forEach(to -> referenceBuilder.getReference(to,
    // cobolDataStructure).resolve().set(fromReference));
  }
}
