...
public class DefaultFormat1DataStructureBuilder implements Format1DataStructureBuildStrategy {
  @Override
  public Format1DataStructure build(
      CobolParser.DataDescriptionEntryFormat1Context format1Structure,
      UnresolvedReferenceStrategy strategy,
      SourceSection sourceSection) {
    if (format1Structure.dataOccursClause() == null
        || format1Structure.dataOccursClause().isEmpty()) {
      return new Format1DataStructure(format1Structure, strategy, sourceSection);
    }
    int numOccurrences =
        Integer.parseInt(format1Structure.dataOccursClause().getFirst().integerLiteral().getText());
