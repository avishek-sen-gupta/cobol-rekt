...
public class CobolConditionResolver implements ConditionResolver {
  ...
  public static ConditionResolver ALWAYS_YES =
      new CobolConditionResolver(
          AlwaysBooleanResolver.ALWAYS_TRUE,
          AlwaysBooleanResolver.ALWAYS_TRUE,
          AlwaysBooleanResolver.ALWAYS_TRUE);
  public static ConditionResolver ALWAYS_NO =
      new CobolConditionResolver(
          AlwaysBooleanResolver.ALWAYS_FALSE,
          AlwaysBooleanResolver.ALWAYS_FALSE,
          AlwaysBooleanResolver.ALWAYS_FALSE);
  public static ConditionResolver CONSOLE_RESOLVER =
      new CobolConditionResolver(
          ConsoleInputResolver.CONSOLE_INPUT_RESOLVER,
          ConsoleInputResolver.CONSOLE_INPUT_RESOLVER,
          ConsoleInputResolver.CONSOLE_INPUT_RESOLVER);
  public static ConditionResolver EVALUATING_RESOLVER =
      new CobolConditionResolver(
          ExpressionEvaluationResolver.EXPRESSION_EVALUATOR,
          AlwaysBooleanResolver.ALWAYS_TRUE,
          AlwaysBooleanResolver.ALWAYS_TRUE);
  ...
  public CobolConditionResolver(
      BooleanResolver ifResolver, BooleanResolver whenResolver, BooleanResolver onResolver) {
  ...
  public boolean resolveIf(FlowNode node, FlowNodeService nodeService) {
    ...
    LOGGER.finer(
        ConsoleColors.green(
            "Resolved " + formatted(node.label()) + " condition to " + result + "..."));
    return result;
  }
  ...
  public boolean resolveWhen(FlowNode whenPhrase, FlowNodeService nodeService) {
    ...
    boolean result =
        whenResolver.resolve(
            whenPhrase,
            (CobolParser.ConditionContext) whenNode.getCondition().getExecutionContext(),
            nodeService);
    LOGGER.finer(
        ConsoleColors.green(
            "Resolved WHEN condition: " + formatted(whenPhrase.label()) + " to " + result + "..."));
    return result;
  }
