package org.smojol.toolkit.interpreter.stack;
...
public class CobolStackFrames implements StackFrames {
  ...
  @Deprecated
  public CobolStackFrames(List<IStackFrame> frames) {
  ...
  public StackFrames add(FlowNode frameContext) {
    ...
    shallowCopy.add(
        new CobolStackFrame(
            frameContext, frames.isEmpty() ? dataStructures : frames.getLast().dataStructures()));
  ...
  public CobolVmSignal callSite() {
    ...
    while (i >= 0 && frames.get(i).isOfType(GoToFlowNode.class)) {
      //            if (frames.get(i).getClass() == PerformProcedureChartNode.class) return
      // CobolVmSignal.EXIT_PERFORM;
