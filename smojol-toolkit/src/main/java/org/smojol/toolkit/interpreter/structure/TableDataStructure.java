...
public class TableDataStructure extends Format1DataStructure {
  ...
  public TableDataStructure(
      CobolParser.DataDescriptionEntryFormat1Context structure,
      int numElements,
      UnresolvedReferenceStrategy strategy,
  ...
  // Copy constructor
  //    public TableDataStructure(Function<CobolParser.DataDescriptionEntryFormat1Context, String>
  ...
  // List<ConditionalDataStructure> conditions, int numElements, SourceSection sourceSection) {
  //        super(namingScheme, dataDescription, childStructures, level, parent, isComposite,
  // unresolvedReferenceStrategy, conditions, CobolDataType.TABLE, sourceSection);
  //        this.numElements = numElements;
  //    }

  // Copy constructor
  public TableDataStructure(
      TableDataStructure that,
  ...
  public void expandTables() {
    if (!isComposite) {
      //            structures = IntStream.range(0, numElements).mapToObj(i -> (CobolDataStructure)
      // new Format1DataStructure(this, NamingScheme.INDEXED.apply(i))).toList();
      structures =
          IntStream.range(0, numElements)
              .mapToObj(
                  i ->
                      (CobolDataStructure)
                          new Format1DataStructure(
                              this.dataDescription,
                              elementType,
                              unresolvedReferenceStrategy,
                              sourceSection))
              .toList();
    } else {
      ...
      structures =
          IntStream.range(0, numElements)
              .mapToObj(
                  i -> {
                    List<CobolDataStructure> copiedStructures =
                        structures.stream().map(s -> s.copy(NamingScheme.IDENTITY)).toList();
                    return (CobolDataStructure)
                        new Format1DataStructure(this, copiedStructures, NamingScheme.IDENTITY);
                  })
              .toList();
    }
  }
  ...
  public CobolDataStructure copy(
    ...
    //        return new TableDataStructure(namingScheme, dataDescription, copy(structures),
    // level(), this, isComposite, unresolvedReferenceStrategy, conditions, numElements,
    // sourceSection);
    //        return new Format1DataStructure(namingScheme, dataDescription, copy(structures),
    // level(), this, isComposite, unresolvedReferenceStrategy, conditions, elementType);
  }
