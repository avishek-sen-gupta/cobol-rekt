...
public class MultiplyOperation implements CobolOperation {
  ...
  public void run(CobolDataStructure cobolDataStructure) {
    ...
    CobolExpression sourceProduct =
        multiply.getSourceExpressions().stream()
            .map(srcExpr -> srcExpr.evaluate(cobolDataStructure))
            .reduce(
                new PrimitiveCobolExpression(TypedRecord.typedNumber(1.0)),
                (product, currentExpr) -> product.multiply(currentExpr, cobolDataStructure));
    multiply
        ...
        .forEach(
            to ->
                builder
                    ...
                    .set(builder.getReference(sourceProduct, cobolDataStructure)));
  }
}
