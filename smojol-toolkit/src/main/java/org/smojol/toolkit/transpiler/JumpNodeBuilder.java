...
public class JumpNodeBuilder {
  public static TranspilerNode build(GoToFlowNode n, CobolDataStructure dataStructures) {
    if (n.callTargets() == null)
      throw new RuntimeException("Call targets are empty: " + n.originalText());
    if (!n.dependsUponFactor())
      return new JumpTranspilerNode(new NamedLocationNode(n.callTargets().getFirst().name()));
  ...
  private static TranspilerNode ladder(GoToFlowNode n, ValueOfNode factorValue, int index) {
    if (index == n.callTargets().size() - 1)
      return new IfTranspilerNode(
          new EqualToNode(
              factorValue, new PrimitiveValueTranspilerNode(TypedRecord.typedNumber(index + 1))),
          new DetachedTranspilerCodeBlockNode(
              ImmutableList.of(
                  new JumpTranspilerNode(
                      new NamedLocationNode(n.callTargets().get(index).name())))),
          new DetachedTranspilerCodeBlockNode());
    return new IfTranspilerNode(
        new EqualToNode(
            factorValue, new PrimitiveValueTranspilerNode(TypedRecord.typedNumber(index + 1))),
        new DetachedTranspilerCodeBlockNode(
            ImmutableList.of(
                new JumpTranspilerNode(new NamedLocationNode(n.callTargets().get(index).name())))),
