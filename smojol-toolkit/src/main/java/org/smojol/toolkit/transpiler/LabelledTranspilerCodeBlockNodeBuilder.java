...
public class LabelledTranspilerCodeBlockNodeBuilder {
  public static TranspilerNode build(
    ...
    return labelledBlock(
        n,
        dataStructures,
        ImmutableMap.of("type", FlowNodeType.SECTION),
        sectionParagraphMap,
        next);
  }

  private static LabelledTranspilerCodeBlockNode labelledBlock(
      FlowNode n,
      CobolDataStructure dataStructures,
      Map<String, Object> properties,
      SectionParagraphMap sectionParagraphMap,
      FlowNode next) {
    Stream<TranspilerNode> childTranspilerNodes =
        n.astChildren().stream()
            .map(
                child ->
                    TranspilerTreeBuilder.flowToTranspiler(
                        child, dataStructures, sectionParagraphMap));
    List<TranspilerNode> childNodesWithRetCall =
        Stream.concat(
                childTranspilerNodes,
                next instanceof NullFlowNode
                    ? Stream.of()
                    : Stream.of(new RetCallTranspilerNode(next.label())))
            .toList();
  ...
  public static TranspilerNode build(
      ProcedureDivisionBodyFlowNode n,
      CobolDataStructure dataStructures,
      SectionParagraphMap sectionParagraphMap) {
    return labelledBlock(
        n,
        dataStructures,
        ImmutableMap.of("type", FlowNodeType.PROCEDURE_DIVISION_BODY),
        sectionParagraphMap,
        new NullFlowNode());
  }
  ...
  // {
  //        List<TranspilerNode> childTranspilerNodes = n.astChildren().stream().map(child ->
  // TranspilerTreeBuilder.flowToTranspiler(child, dataStructures, sectionParagraphMap)).toList();
