...
public class SubtractTranspilerNodeBuilder {
    public static TranspilerNode build(SubtractFlowNode n, CobolDataStructure dataStructures) {
        ...
        if (!n.isGiving())
            return new TranspilerCodeBlockNode(zip(n.getDestinationExpressions().stream().map(nodeBuilder::build), differences.stream(), (dst, src) -> (TranspilerNode) new SetTranspilerNode(src, dst)).toList());
        return new TranspilerCodeBlockNode(n.getDestinationExpressions().stream().map(dst -> (TranspilerNode) new SetTranspilerNode(differences.getFirst(), nodeBuilder.build(dst))).toList());
    }
}
