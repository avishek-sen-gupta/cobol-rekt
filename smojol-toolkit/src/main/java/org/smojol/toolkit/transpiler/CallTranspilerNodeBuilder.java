...
public class CallTranspilerNodeBuilder {
  public static TranspilerNode build(CallTarget callTarget) {
    return switch (callTarget) {
      case StaticCallTarget sct ->
          new CallTranspilerNode(
              new PrimitiveValueTranspilerNode(TypedRecord.typedString(sct.getName())));
      case DynamicCallTarget dct ->
          new CallTranspilerNode(new ValueOfNode(new SymbolReferenceNode(dct.getName())));
