...
public class MultiplyTranspilerNodeBuilder {
  public static TranspilerNode build(MultiplyFlowNode n, CobolDataStructure dataStructures) {
    ...
    TranspilerNode product =
        n.getSourceExpressions().stream().map(nodeBuilder::build).reduce(MultiplyNode::new).get();
    if (!n.isGiving())
      return new TranspilerCodeBlockNode(
          n.getDestinationExpressions().stream()
              .map(dst -> (TranspilerNode) new SetTranspilerNode(product, nodeBuilder.build(dst)))
              .toList());
    return new TranspilerCodeBlockNode(
        n.getDestinationExpressions().stream()
            .map(dst -> (TranspilerNode) new SetTranspilerNode(product, nodeBuilder.build(dst)))
