...
public class EvaluateNodeTranslator {
  public static TranspilerNode build(
    ...
    TranspilerCodeBlockNode elseBody =
        new DetachedTranspilerCodeBlockNode(
            deconstructedRepresentation.elseBody().stream()
                .map(
                    stmt ->
                        TranspilerTreeBuilder.flowToTranspiler(
                            stmt, dataStructures, sectionParagraphMap))
                .toList());
    return recursiveOr(head(clauses), tail(clauses), dataStructures, elseBody, sectionParagraphMap);
  }

  private static TranspilerNode recursiveOr(
      Optional<TestActionPair> current,
      List<TestActionPair> remaining,
      CobolDataStructure dataStructures,
      TranspilerCodeBlockNode elseBody,
      SectionParagraphMap sectionParagraphMap) {
    ...
    TranspilerCodeBlockNode transpilerBody =
        new DetachedTranspilerCodeBlockNode(
            bodyStatements.stream()
                .map(
                    stmt ->
                        TranspilerTreeBuilder.flowToTranspiler(
                            stmt, dataStructures, sectionParagraphMap))
    ...
    if (remaining.isEmpty())
      return new IfTranspilerNode(builder.build(test), transpilerBody, elseBody);
    return new IfTranspilerNode(
        builder.build(test),
        transpilerBody,
        recursiveOr(
            head(remaining), tail(remaining), dataStructures, elseBody, sectionParagraphMap));
  }
}
