...
public class InterpretTask implements AnalysisTask {
  private static final java.util.logging.Logger LOGGER =
      Logger.getLogger(InterpretTask.class.getName());
  ...
  public AnalysisTaskResult run() {
    ...
    try {
      ...
      FlowNode root =
          new BuildFlowNodesTask(
                  new FlowNodeServiceImpl(navigator, dataStructures, ops.getIdProvider()))
              .run(procedure);
      //            flowcharter.buildFlowAST(procedure).buildControlFlow().buildOverlay();
      //            FlowNode root = flowcharter.getRoot();
      root.resolve(
          new SmojolSymbolTable(dataStructures, new SymbolReferenceBuilder(ops.getIdProvider())),
          dataStructures);

      //            new FlowNodeASTTraversal<FlowNode>().accept(sourceGraphRoot, new
      // FlowNodeSymbolExtractorVisitor(sourceGraphRoot, dataStructures, new
      // SmojolSymbolTable(dataStructures, new SymbolReferenceBuilder(new UUIDProvider()))));
      LOGGER.info("DATA STRUCTURES\n--------------------------------\n");
      dataStructures.report();
      LOGGER.info("INTERPRETING\n--------------------------------\n");
      Breakpointer bp = new CobolBreakpointer();
      bp.addBreakpoint(
          n -> n.getClass() == DisplayFlowNode.class && n.originalText().contains("SOMETEXT"));
      root.acceptInterpreter(
          CobolInterpreterFactory.executingInterpreter(
              conditionResolver,
              dataStructures,
              ImmutableList.of(),
              new ExecutionListeners(
                  ImmutableList.of(new RunLogger("/Users/asgupta/code/smojol/out/report.md"))),
              bp),
