...
public class DataRedefinitionComputer {
  public static Map.Entry<CobolDataStructure, CobolDataStructure> redefinitionPair(
      CobolDataStructure redefiningRecord, CobolDataStructure dataRoot) {
    ...
    CobolParser.DataDescriptionEntryFormat1Context dataDescription =
        ((Format1DataStructure) redefiningRecord).getDataDescription();
    if (dataDescription == null || dataDescription.dataRedefinesClause().isEmpty())
      return ImmutablePair.nullPair();
    String redefinedRecordName =
        dataDescription.dataRedefinesClause().getFirst().dataName().getText();
