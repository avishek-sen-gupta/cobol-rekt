...
public class SerialisableCFGGraphCollector implements FlowNodeVisitor {
  ...
  public void visit(
    ...
    edges.addAll(
        outgoingNodes.stream()
            .map(
                o ->
                    new SerialisableEdge(
                        idProvider.next(), node.id(), o.id(), NodeRelations.FOLLOWED_BY))
            .toList());
  }
  ...
  public void visitParentChildLink(
    ...
    edges.add(
        new SerialisableEdge(
            idProvider.next(), parent.id(), internalTreeRoot.id(), NodeRelations.STARTS_WITH));
  }

  @Override
  public void visitControlTransfer(FlowNode from, FlowNode to, VisitContext visitContext) {
    edges.add(new SerialisableEdge(idProvider.next(), from.id(), to.id(), NodeRelations.JUMPS_TO));
  }
