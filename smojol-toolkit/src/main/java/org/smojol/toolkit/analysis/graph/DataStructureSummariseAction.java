...
public class DataStructureSummariseAction implements NodeAction {
  ...
  public ActionResult apply(Record node, List<ActionResult> childResults) {
    ...
    String s =
        NodeAccess.name(node)
            + " is of type "
            + NodeAccess.type(node)
            + " and is composed of ["
            + String.join(",", childStrings)
            + "]";
    String prompt =
        "You are a domain expert. This is a variable associated with workflows in this domain. Without any extra text, deduce what this variable represents. Be as precise as possible. Summaries of child data structures follow: "
            + s;
