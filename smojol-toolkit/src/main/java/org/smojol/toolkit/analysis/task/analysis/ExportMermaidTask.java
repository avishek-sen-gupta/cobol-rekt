...
public class ExportMermaidTask implements AnalysisTask {
  ...
  public AnalysisTaskResult run() {
    List<FlowNode> sections =
        new FlowNodeNavigator(flowRoot).findAllByCondition(fn -> fn.type() == FlowNodeType.SECTION);
    try {
  ...
  private void writeCodeBlock(FlowNode s) {
    ...
    String sectionOutputPath =
        Paths.get(mermaidOutputConfig.outputDir().toString(), filename + ".md").toString();
