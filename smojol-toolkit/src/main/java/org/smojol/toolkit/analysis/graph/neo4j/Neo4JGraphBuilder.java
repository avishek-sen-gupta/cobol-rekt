...
public class Neo4JGraphBuilder {
  ...
  public void buildAST(FlowNode node) {
    ...
    new FlowNodeASTTraversal<Record>()
        .accept(node, new Neo4JASTBuilderVisitor(astNodeReferenceStrategy, sdk, qualifier));
  }
  ...
  public void buildDataDependencies(FlowNode root) {
    ...
    new FlowNodeASTTraversal<Record>()
        .accept(
            root,
            new Neo4JDataDependencyBuilderVisitor(
                data, sdk, qualifier, dependencyAttachmentStrategy));
  }
}
