...
public class IdmsCopyBookTreeVisualiser {
   ...
   */
  public void visualiseIdmsAST(
      IdmsCopyParser.StartRuleContext startRuleContext, String idmsParseTreeOutputPath) {
    Gson gson =
  ...
  private CobolContextAugmentedTreeNode drawParseTrees(ParseTree parseRuleContext) {
    CobolContextAugmentedTreeNode graphRoot =
        new CobolContextAugmentedTreeNode(parseRuleContext, null);
  ...
  private void buildGraph(ParseTree astParentNode, CobolContextAugmentedTreeNode graphParentNode) {
    for (int i = 0; i <= astParentNode.getChildCount() - 1; ++i) {
      ...
      CobolContextAugmentedTreeNode graphChildNode =
          new CobolContextAugmentedTreeNode(astChildNode, null);
