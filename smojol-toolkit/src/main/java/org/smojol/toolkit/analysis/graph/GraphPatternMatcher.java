...
public class GraphPatternMatcher implements FlowNodeVisitor {
  ...
  @Getter private final List<List<FlowNode>> matches = new ArrayList<>();
  ...
  public void visit(
    ...
    if (node.type() == FlowNodeType.SENTENCE
        && node.astChildren().size() == 1
        && node.astChildren().getFirst().type() == FlowNodeType.MOVE) {
      registerMove(node);
      return;
    }
