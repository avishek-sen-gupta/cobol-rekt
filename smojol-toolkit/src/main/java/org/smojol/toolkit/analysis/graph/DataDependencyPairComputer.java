...
public class DataDependencyPairComputer {
  ...
  // TODO: This can be rewritten bottom-up by finding all references, and then working our way up to
  // a statement or a sentence
  public static Pair<List<CobolDataStructure>, List<CobolDataStructure>> dependencyPairs(
      FlowNode node, CobolDataStructure dataRoot) {
    ...
    if (node.type() == FlowNodeType.IF_BRANCH) {
      ...
      List<CobolDataStructure> conditionStructures =
          staticExpressionsFromSingle(expression, dataRoot);
      return ImmutablePair.of(conditionStructures, ImmutableList.of());
    }
    if (node.type() == FlowNodeType.MOVE) {
      MoveFlowNode move = (MoveFlowNode) node;
      List<CobolDataStructure> froms =
          staticExpressionsFromSingle(move.getFromExpressions().getFirst(), dataRoot);
      ...
      List<CobolDataStructure> froms =
          staticExpressionsFromSingle(compute.getRhsExpression(), dataRoot);
      List<CobolDataStructure> tos =
          staticExpressionsFromMany(compute.getDestinationExpressions(), dataRoot);
      return ImmutablePair.of(froms, tos);
      ...
      List<CobolDataStructure> minuends =
          staticExpressionsFromMany(subtract.getMinuendExpressions(), dataRoot);
      List<CobolDataStructure> subtrahends =
          staticExpressionsFromMany(subtract.getSubtrahendExpressions(), dataRoot);
      ...
      List<CobolDataStructure> lhses =
          staticExpressionsFromSingle(multiply.getLhsExpression(), dataRoot);
      List<CobolDataStructure> rhses =
          staticExpressionsFromMany(multiply.getRhsExpressions(), dataRoot);
      return ImmutablePair.of(lhses, rhses);
    } else if (node.type() == FlowNodeType.DIVIDE) {
      DivideFlowNode divide = (DivideFlowNode) node;
      List<CobolDataStructure> dividends =
          staticExpressionsFromMany(divide.getDividendExpressions(), dataRoot);
      List<CobolDataStructure> divisors =
          staticExpressionsFromSingle(divide.getDivisorExpression(), dataRoot);
      return ImmutablePair.of(divisors, dividends);
    }
    return ImmutablePair.nullPair();
  }

  private static List<CobolDataStructure> staticExpressionsFromMany(
      List<CobolExpression> expressions, CobolDataStructure dataRoot) {
    return expressions.stream()
        .flatMap(minuend -> staticExpressionsFromSingle(minuend, dataRoot).stream())
        .toList();
  }

  private static List<CobolDataStructure> staticExpressionsFromSingle(
      CobolExpression expression, CobolDataStructure dataRoot) {
