...
public class BuildTranspilerFlowgraphTask {
  ...
  public TranspilerFlowgraph run() {
    TranspilerNode transpilerTree =
        new BuildTranspilerASTTask(rawAST, dataStructures, symbolTable).run();

    //        TranspilerNode mainNode = transpilerTree.findAllRecursive(t -> t instanceof
    // LabelledTranspilerCodeBlockNode l &&
    // l.getName().equals("S-STEUERUNGS")).stream().findFirst().get();
    List<TranspilerInstruction> instructions =
        new BuildTranspilerInstructionsFromIntermediateTreeTask(
                transpilerTree, new IncrementingIdProvider())
            .run();
    Graph<TranspilerInstruction, DefaultEdge> implicitCFG =
        new BuildImplicitInstructionControlFlowgraphTask(instructions, ImmutableList.of()).run();
    Set<InvokingProcedureRange> rangesWithChildren =
        new ProcedureBodyTask(transpilerTree, instructions, implicitCFG).run();
    ...
    Graph<TranspilerInstruction, DefaultEdge> instructionFlowgraph =
        new BuildInstructionFlowgraphTask(instructions, flowHints).run();
    Pair<
            Graph<BasicBlock<TranspilerInstruction>, DefaultEdge>,
            List<BasicBlock<TranspilerInstruction>>>
        basicBlockModel =
            new BuildBasicBlocksTask(
                    instructions,
                    instructionFlowgraph,
                    new BasicBlockFactory<>(new IncrementingIdProvider()),
                    neo4JDriverBuilder)
                .run();
    Graph<BasicBlock<TranspilerInstruction>, DefaultEdge> basicBlockGraph =
        basicBlockModel.getLeft();
    MermaidGraph<TranspilerInstruction, DefaultEdge> mermaid = new MermaidGraph<>();
    String draw = mermaid.draw(instructionFlowgraph);
    return new TranspilerFlowgraph(
        basicBlockGraph,
        instructionFlowgraph,
        transpilerTree,
        instructions,
        basicBlockModel.getRight(),
        categorisedRanges);
    ...
    // CommandLineAnalysisTask.BUILD_TRANSPILER_FLOWGRAPH);
    //        }
    //
    //        try (JsonWriter writer = new
    // JsonWriter(resourceOperations.fileWriter(transpilerModelOutputConfig.fullPath()))) {
    //            Gson gson = initGson();
    //            writer.setIndent("  ");
    ...
    //            return AnalysisTaskResult.OK(CommandLineAnalysisTask.BUILD_TRANSPILER_FLOWGRAPH,
    // new TranspilerFlowgraph(basicBlockGraph, instructionFlowgraph));
    //        } catch (IOException e) {
    //            return AnalysisTaskResult.ERROR(e,
    // CommandLineAnalysisTask.BUILD_TRANSPILER_FLOWGRAPH);
    //        }
  ...
  private Record recursivelyInject(
      TranspilerNode current, GraphSDK graphSDK, NodeSpecBuilder specBuilder) {
    ...
    List<Record> childRecords =
        current.astChildren().stream()
            .map(c -> recursivelyInject(c, graphSDK, specBuilder))
            .toList();
    List<Record> internalRecords =
        current.internalElements().stream()
            .map(c -> recursivelyInject(c, graphSDK, specBuilder))
            .toList();
  ...
  public static Gson initGson() {
    RuntimeTypeAdapterFactory<TranspilerNode> runtimeTypeAdapterFactory =
        RuntimeTypeAdapterFactory.of(TranspilerNode.class, "type")
