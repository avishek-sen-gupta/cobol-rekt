...
public class SerialisableNodeOperation {
  ...
  public SerialisableNodeOperation(ZhangShashaTreeEditDistance.EditOperation<TypedGraphVertex> op) {
    ...
    firstOperand =
        ImmutableMap.of(
            "name",
            op.getFirstOperand().name(),
            "type",
            op.getFirstOperand().type(),
            "text",
            op.getFirstOperand().text());
    secondOperand =
        op.getSecondOperand() == null
            ? ImmutableMap.of()
            : ImmutableMap.of(
                "name",
                op.getSecondOperand().name(),
                "type",
                op.getSecondOperand().type(),
                "text",
                op.getSecondOperand().text());
  }
}
