...
public class WriteFlowASTTask implements AnalysisTask {
  ...
  public AnalysisTaskResult run() {
    SerialisableASTFlowNode serialisableASTFlowRoot =
        new SerialiseFlowASTTask().serialisedFlowAST(astRoot);
    ...
    try (JsonWriter writer =
        new JsonWriter(resourceOperations.fileWriter(flowASTOutputConfig.outputPath()))) {
      //        try (JsonWriter writer = new JsonWriter(new
      // FileWriter(flowASTOutputConfig.outputPath()))) {
      writer.setIndent("  ");
