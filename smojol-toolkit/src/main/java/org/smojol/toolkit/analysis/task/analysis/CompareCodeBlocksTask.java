...
public class CompareCodeBlocksTask {
  ...
  public AnalysisTaskResult run(FlowNode astRoot, NodeOperationCostFunctions costFunctionBlock) {
    List<FlowNode> sectionNodes =
        new FlowNodeNavigator(astRoot)
            .findAllByCondition(n -> n.getClass() == ParagraphFlowNode.class);
    HashMap<FlowNode, Pair<TypedGraphVertex, Graph<TypedGraphVertex, TypedGraphEdge>>>
        flowJGraphTPairs = new HashMap<>();
    sectionNodes.forEach(
        n -> {
          JGraphTGraphBuilder graphMLExporter =
              new JGraphTGraphBuilder(
                  dataStructures, n, qualifier, new DefaultUndirectedGraph<>(TypedGraphEdge.class));
          graphMLExporter.buildAST();
          Graph<TypedGraphVertex, TypedGraphEdge> model = graphMLExporter.getModel();
          TypedGraphVertex root =
              model.vertexSet().stream()
                  .filter(v -> FlowNodeType.PARAGRAPH.name().equals(v.type()))
                  .toList()
                  .getFirst();
    ...
    List<SimilarityResult> allDistances =
        allComparandFlowPairs.stream()
            .map(
                p -> {
                  Pair<TypedGraphVertex, Graph<TypedGraphVertex, TypedGraphEdge>> leftGraphTModel =
                      flowJGraphTPairs.get(p.getLeft());
                  Pair<TypedGraphVertex, Graph<TypedGraphVertex, TypedGraphEdge>> rightGraphTModel =
                      flowJGraphTPairs.get(p.getRight());
                  ...
                  ZhangShashaTreeEditDistance<TypedGraphVertex, TypedGraphEdge> editDistance =
                      new ZhangShashaTreeEditDistance<>(
                          ...
                          rightModel,
                          rightRoot,
                          costFunctionBlock.add(),
                          costFunctionBlock.remove(),
                          costFunctionBlock.change());
                  //            return ImmutablePair.of(editDistance.getEditOperationLists(),
                  // editDistance.getDistance());
                  return new SimilarityResult(
                      p, editDistance.getDistance(), editDistance.getEditOperationLists());
                })
            .toList();
    allDistances.forEach(
        d -> {
          LOGGER.info(
              String.format(
                  "Distance between %s and %s is %s",
                  d.nodes().getLeft(), d.nodes().getRight(), d.distance()));
        });
  ...
  private List<Pair<FlowNode, FlowNode>> recurse(List<FlowNode> paragraphs) {
    ...
    List<ImmutablePair<FlowNode, FlowNode>> comparands =
        paragraphs.subList(1, paragraphs.size()).stream()
            .map(p -> ImmutablePair.of(paragraphs.getFirst(), p))
            .toList();
    List<Pair<FlowNode, FlowNode>> allComparands = new ArrayList<>();
    List<Pair<FlowNode, FlowNode>> recursedComparands =
        recurse(paragraphs.subList(1, paragraphs.size()));
