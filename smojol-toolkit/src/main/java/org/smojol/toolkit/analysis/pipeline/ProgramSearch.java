...
public class ProgramSearch {
  ...
  public static final Function2<String, File, Boolean> STRICT =
      (name, f) -> name.equals(f.getName());
  public static final Function2<String, File, Boolean> PERMISSIVE =
      (name, f) ->
  ...
  public ProgramSearch(
      Function2<String, File, Boolean> matchStrategy, ResourceOperations resourceOperations) {
  ...
  public Pair<File, String> run(String program, String sourceDir) {
    ...
    if (matchingFiles.isEmpty()) {
      LOGGER.info(
          ConsoleColors.red(
              "No files matching " + program + " in " + sourceDir + ". Terminating recursion..."));
