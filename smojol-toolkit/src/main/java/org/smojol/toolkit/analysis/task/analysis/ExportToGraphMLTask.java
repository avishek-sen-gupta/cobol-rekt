...
public class ExportToGraphMLTask implements AnalysisTask {
  ...
  public AnalysisTaskResult run() {
    try {
      ...
      String graphMLOutputPath =
          graphMLOutputConfig
              .outputDir()
              .resolve(graphMLOutputConfig.outputPath())
              .toAbsolutePath()
              .normalize()
              .toString();
      //            new FlowNodeASTTraversal<FlowNode>().accept(astRoot, new
      // FlowNodeSymbolExtractorVisitor(astRoot, dataStructures, null));
      Graph<TypedGraphVertex, TypedGraphEdge> model =
  ...
  private static Graph<TypedGraphVertex, TypedGraphEdge> exportUnifiedToGraphML(
      FlowNode astRoot,
      CobolDataStructure dataStructures,
      NodeSpecBuilder qualifier,
      String outputPath) {
    JGraphTGraphBuilder graphMLExporter =
        new JGraphTGraphBuilder(dataStructures, astRoot, qualifier);
