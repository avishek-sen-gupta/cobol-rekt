...
public class SummariseThroughLLMTask implements AnalysisTask {
  ...
  private static void summariseThroughLLM(NodeSpecBuilder qualifier, GraphSDK sdk) {
    Record neo4jProgramRoot =
        sdk.findNodes(
                qualifier.cfgNodeCriteria(
                    Map.of(TYPE, FlowNodeType.PROCEDURE_DIVISION_BODY.toString())))
            .getFirst();
    Record neo4jDataStructuresRoot =
        sdk.findNodes(qualifier.dataNodeSearchCriteria(Map.of(SECTION_SOURCE, "ROOT"))).getFirst();
    ...
    new TreeMapperTraversal<Record, ActionResult>()
        .accept(neo4jProgramRoot, new Neo4JCodeSummaryVisitor(advisor, sdk), codeChildrenFn);
    new TreeMapperTraversal<Record, ActionResult>()
        .accept(neo4jDataStructuresRoot, new Neo4JDataSummaryVisitor(advisor, sdk), dataChildrenFn);
  }
}
