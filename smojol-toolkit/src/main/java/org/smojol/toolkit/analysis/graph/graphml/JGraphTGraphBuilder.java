...
public class JGraphTGraphBuilder {
  ...
  public JGraphTGraphBuilder(
      CobolDataStructure dataStructures, FlowNode procedureRoot, NodeSpecBuilder qualifier) {
  ...
  public void buildAST() {
    ...
    JGraphTASTGraphBuilderVisitor jGraphTASTGraphBuilderVisitor =
        new JGraphTASTGraphBuilderVisitor(astGraphOperations);
  ...
  public void buildDataStructures() {
    dataRoot.accept(
        new GraphMLDataStructureVisitor(dataStructuresGraph, qualifier),
        null,
        n -> false,
        dataRoot);
    dataRoot.accept(
        new GraphMLRedefinitionVisitor(dataStructuresGraph, qualifier), null, n -> false, dataRoot);
    //        new FlowNodeASTTraversal<Boolean>().build(astRoot, this::buildDataDependency);
    new FlowNodeASTTraversal<CobolDataStructure>()
        .accept(astRoot, new JGraphTDataDependencyBuilderVisitor(dataGraphOperations, dataRoot));
  }
  ...
  public SerialisableUnifiedModel asSerialisable() {
    List<SerialisableCFGFlowNode> serialisableCodeVertices =
        astGraph.vertexSet().stream()
            .filter(v -> v.getClass() == TypedCodeVertex.class)
            .map(c -> new SerialisableCFGFlowNode(((TypedCodeVertex) c).getNode()))
            .toList();
    List<SerialisableCobolDataStructure> serialisableDataVertices =
        astGraph.vertexSet().stream()
            .filter(v -> v.getClass() == TypedDataStructureVertex.class)
            .map(c -> new SerialisableCobolDataStructure(((TypedDataStructureVertex) c).getNode()))
            .toList();
    List<SerialisableEdge> serialisableEdges =
        astGraph.edgeSet().stream()
            .map(
                e -> {
                  TypedGraphVertex from = astGraph.getEdgeSource(e);
                  TypedGraphVertex to = astGraph.getEdgeTarget(e);
                  return new SerialisableEdge(
                      UUID.randomUUID().toString(), from.id(), to.id(), e.getRelationshipType());
                })
            .toList();
    return new SerialisableUnifiedModel(
        serialisableCodeVertices, serialisableDataVertices, serialisableEdges);
  }
  ...
  public void writeToGraphML(File outputPath) {
    ...
    exporter.registerAttribute(
        NAMESPACE, GraphMLExporter.AttributeCategory.NODE, AttributeType.STRING);
    //        exporter.registerAttribute(NAMESPACE, GraphMLExporter.AttributeCategory.EDGE,
    // AttributeType.STRING);
    exporter.registerAttribute(
        RELATIONSHIP_TYPE, GraphMLExporter.AttributeCategory.EDGE, AttributeType.STRING);
  ...
  public void runAlgo(File outputPath) {
    ...
    BronKerboschCliqueFinder<TypedGraphVertex, TypedGraphEdge> cliqueFinder =
        new BronKerboschCliqueFinder<>(undirected);
    ArrayList<Set<TypedGraphVertex>> cliques = new ArrayList<>();
    cliqueFinder.forEach(cliques::add);
    KSpanningTreeClustering<TypedGraphVertex, TypedGraphEdge> clusterer =
        new KSpanningTreeClustering<>(undirected, 50);
