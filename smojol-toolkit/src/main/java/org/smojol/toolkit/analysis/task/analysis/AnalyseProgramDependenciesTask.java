...
public class AnalyseProgramDependenciesTask {
  java.util.logging.Logger LOGGER =
      Logger.getLogger(AnalyseProgramDependenciesTask.class.getName());
  ...
  public AnalyseProgramDependenciesTask(
      String sourceDir,
    ...
    try {
      ...
      Map<String, List<AnalysisTaskResult>> results =
          new CodeTaskRunner(
              ...
              .runForPrograms(
                  ImmutableList.of(
                      BUILD_BASE_ANALYSIS, CommandLineAnalysisTask.BUILD_PROGRAM_DEPENDENCIES),
                  ImmutableList.of(foundFile.getName()));
      AnalysisTaskResult first = results.get(program.getName()).get(1);
      List<CallTarget> dependencies =
          switch (first) {
      ...
      LOGGER.severe(
          ConsoleColors.red(
              "Error, terminating recursion down this path. Error: "
