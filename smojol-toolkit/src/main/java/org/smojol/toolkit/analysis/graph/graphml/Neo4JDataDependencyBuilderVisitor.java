...
public class Neo4JDataDependencyBuilderVisitor extends FlowNodeASTVisitor<Record> {
  ...
  public Neo4JDataDependencyBuilderVisitor(
      CobolDataStructure dataRoot,
      GraphSDK graphSDK,
      NodeSpecBuilder qualifier,
      NodeReferenceStrategy dependencyAttachmentStrategy,
      Record ancestor) {
    super(ancestor);
    this.sdk = graphSDK;
  ...
  public Record visit(FlowNode node) {
    Map.Entry<List<CobolDataStructure>, List<CobolDataStructure>> pairs =
        DataDependencyPairComputer.dependencyPairs(node, dataRoot);
  ...
  private void accesses(FlowNode attachmentNode, List<CobolDataStructure> dataNodes) {
    ...
    Record attachmentNodeRecord =
        dependencyAttachmentStrategy.reference(attachmentNode, sdk, qualifier);
    dataNodes.forEach(
        n -> {
          Record n4jFrom =
              sdk.newOrExisting(
                  qualifier.dataNodeSearchSpec(n), NodeToWoof.dataStructureToWoof(n, qualifier));
          sdk.accesses(attachmentNodeRecord, n4jFrom);
        });
  }

  private void connect(
      List<CobolDataStructure> froms, List<CobolDataStructure> tos, FlowNode attachmentNode) {
    Record attachmentNodeRecord =
        dependencyAttachmentStrategy.reference(attachmentNode, sdk, qualifier);
    tos.forEach(
        to -> {
          ...
          Record n4jTo =
              !nodes.isEmpty()
                  ? nodes.getFirst()
                  : sdk.createNode(NodeToWoof.dataStructureToWoof(to, qualifier));
          sdk.modifies(attachmentNodeRecord, n4jTo);
          froms.forEach(
              from -> {
                Record n4jFrom =
                    sdk.newOrExisting(
                        qualifier.dataNodeSearchSpec(from),
                        NodeToWoof.dataStructureToWoof(from, qualifier));
