...
public class CompareCodeTask implements AnalysisTask {
  ...
  public AnalysisTaskResult run() {
    AnalysisTaskResult result =
        new CompareCodeBlocksTask(dataStructures, qualifier)
            .run(astRoot, NodeOperationCostFunctions.GENERIC);
  ...
  private AnalysisTaskResult exportToJSON(List<SimilarityResult> similarityResults) {
    List<SerialisableSimilarityResult> serialisableResults =
        similarityResults.stream()
            .map(
                s ->
                    new SerialisableSimilarityResult(
                        s.nodes(), s.distance(), s.editOperationLists()))
            .toList();
