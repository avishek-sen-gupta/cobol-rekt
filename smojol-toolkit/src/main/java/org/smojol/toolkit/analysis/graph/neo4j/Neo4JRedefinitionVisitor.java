...
public class Neo4JRedefinitionVisitor implements DataStructureVisitor {
  ...
  @Override
  public CobolDataStructure visit(
      CobolDataStructure data, CobolDataStructure parent, CobolDataStructure root) {
    Map.Entry<CobolDataStructure, CobolDataStructure> redefinitionPair =
        DataRedefinitionComputer.redefinitionPair(data, root);
    if (ImmutablePair.nullPair().equals(redefinitionPair)) return data;
    Record redefinition =
        sdk.findNodes(nodeQualifier.dataNodeSearchSpec(redefinitionPair.getKey())).getFirst();
    Record redefinedRecord =
        sdk.findNodes(nodeQualifier.dataNodeSearchSpec(redefinitionPair.getValue())).getFirst();
