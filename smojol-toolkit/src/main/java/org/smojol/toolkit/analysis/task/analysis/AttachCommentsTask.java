  ...
  public AnalysisTaskResult run() {
    try {
      List<CommentBlock> commentBlocks =
          new CommentExtraction().run(sourceConfig.sourcePath(), navigator);
      commentBlocks.forEach(
          cb -> {
            LOGGER.finer("Attaching comments");
            FlowNode node =
                new FlowNodeNavigator(astRoot)
                    .findNarrowestByCondition(
                        n -> n.originalText().contains(cb.getCodeContextLine()));
            if (node != null) node.addComment(cb);
            else {
              CobolDataStructure dataStructure =
                  new DataStructureNavigator(dataStructures)
                      .findByCondition(ds -> ds.getRawText().contains(cb.getCodeContextLine()));
              if (dataStructure != null) dataStructure.addComment(cb);
              else {
                FlowNode possibleIdmsNode =
                    new FlowNodeNavigator(astRoot)
                        .findByCondition(n -> n.originalText().contains(cb.getCodeContextLine()));
