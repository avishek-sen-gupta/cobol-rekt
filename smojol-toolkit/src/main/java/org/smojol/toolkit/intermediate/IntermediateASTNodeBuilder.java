...
public class IntermediateASTNodeBuilder {
  ...
  public IntermediateASTNodeBuilder(
      ParseTree codeRoot, CobolDataStructure dataRoot, SmojolSymbolTable symbolTable) {
    this.dataRoot = dataRoot;
    this.codeRoot = codeRoot;
    this.symbolTable = symbolTable;
    nodeService =
        new FlowNodeServiceImpl(
            new CobolEntityNavigator(codeRoot), dataRoot, new IncrementingIdProvider());
  }
  ...
  private FlowNode recursivelyBuildIntermediateNode(
      ParseTree current, FlowNode parent, StackFrames stackFrames) {
    ...
    List<ParseTree> children =
        IntStream.range(0, actualContext.getChildCount())
            .mapToObj(actualContext::getChild)
            .toList();
    List<FlowNode> flowChildren =
        children.stream()
            .map(
                child ->
                    recursivelyBuildIntermediateNode(
                        child, intermediateNode, stackFrames.add(intermediateNode)))
            .toList();
    List<FlowNode> validFlowChildren =
        flowChildren.stream().filter(n -> !(n instanceof NullFlowNode)).toList();
