...
public class SectionParagraphMap {
  ...
  public SectionParagraphMap(FlowNode flowRoot) {
    ...
    sectionParagraphsMap =
        sectionFlowNodes.stream()
            .map(
                sfn ->
                    (Pair<SectionFlowNode, List<ParagraphFlowNode>>)
                        ImmutablePair.of(
                            sfn, new FlowNodeNavigator(sfn).findAllByType(ParagraphFlowNode.class)))
            .collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
    paragraphToSectionMap =
        sectionFlowNodes.stream()
            .flatMap(
                sfn ->
                    new FlowNodeNavigator(sfn)
                        .findAllByCondition(n -> n.type() == FlowNodeType.PARAGRAPH).stream()
                            .map(para -> ImmutablePair.of((ParagraphFlowNode) para, sfn)))
            .collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
  }

  public Optional<SectionFlowNode> nextSection(SectionFlowNode n) {
    Optional<SectionFlowNode> flowNode =
        sectionFlowNodes.stream().filter(sn -> sn == n).findFirst();
    if (flowNode.isEmpty()) return flowNode;
    int nodeIndex = sectionFlowNodes.indexOf(flowNode.get());
    return nodeIndex == sectionFlowNodes.size() - 1
        ? Optional.empty()
        : Optional.of(sectionFlowNodes.get(nodeIndex + 1));
  }
  ...
  public Optional<ParagraphFlowNode> nextParagraph(ParagraphFlowNode n) {
    ...
    return paraIndex == parasForSection.size() - 1
        ? Optional.empty()
        : Optional.of(parasForSection.get(paraIndex + 1));
  }
}
