...
public class NodeSpecBuilder {
  ...
  public NodeSpec newDataNode(CobolDataStructure structure) {
    return new NodeSpec(
        ImmutableList.of(DATA_STRUCTURE, structure.getDataType().abstractType().name()),
        Map.of(
            ID,
            idProvider.next(),
            INTERNAL_ID,
            ...
            structure.content(),
            TYPE,
            structure.getDataType().abstractType().name(),
            ENTITY_TYPE,
            DATA_STRUCTURE,
            ENTITY_CATEGORIES,
            ImmutableList.of(structure.dataCategory().name()),
            LEVEL,
            structure.getLevelNumber(),
            SECTION_SOURCE,
            structure.getSourceSection().name(),
            NAMESPACE,
            namespaceQualifier.getNamespace()));
  }
  ...
  public NodeSpec labelledCodeNode(FlowNodeLike node, String nodeType) {
    return new NodeSpec(
        ImmutableList.of(nodeType, node.type().toString()),
        Map.of(
            ID,
            idProvider.next(),
            INTERNAL_ID,
            ...
            nodeType,
            ENTITY_CATEGORIES,
            node.categories().stream().map(Enum::name).toList(),
            CODE_SENTINEL_TYPE,
            node.codeSentinelType().name(),
            NAMESPACE,
            namespaceQualifier.getNamespace()));
  }
  ...
  public NodeSpec commentNode(CommentBlock comment) {
    ...
    return new NodeSpec(
        ...
        Map.of(
            ...
            COMMENT_NODE,
            ENTITY_CATEGORIES,
            ImmutableList.of(METADATA.name()),
            CODE_SENTINEL_TYPE,
            CodeSentinelType.BODY.name(),
            NAMESPACE,
            namespaceQualifier.getNamespace()));
  }
  ...
  public NodeSpec program(CobolProgram program) {
    ...
    return new NodeSpec(
        ...
        Map.of(
            ...
            "",
            TYPE,
            program.getCallTarget().getProgramReferenceType().name(),
            ENTITY_TYPE,
            PROGRAM_NODE,
            ENTITY_CATEGORIES,
            ImmutableList.of(PROGRAM.name()),
            NAMESPACE,
            namespaceQualifier.getNamespace()));
  }
  ...
  public NodeSpec newTranspilerNode(TranspilerNode node) {
    return new NodeSpec(
        ImmutableList.of("TRANSPILER_NODE"),
        Map.of(
            ID,
            idProvider.next(),
            INTERNAL_ID,
            ...
            "TRANSPILER_NODE",
            ENTITY_CATEGORIES,
            node.getCategories().stream().map(Enum::name).toList(),
            CODE_SENTINEL_TYPE,
            "BODY",
            NAMESPACE,
            namespaceQualifier.getNamespace()));
