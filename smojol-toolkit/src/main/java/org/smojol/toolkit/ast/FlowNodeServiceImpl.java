...
public class FlowNodeServiceImpl implements FlowNodeService {
  ...
  public FlowNodeServiceImpl(
      CobolEntityNavigator navigator, CobolDataStructure dataStructures, IdProvider idProvider) {
  ...
  public FlowNode sectionOrParaWithName(String name) {
    ...
    // Only place where it's acceptable to have null scope since this section/para is not part of
    // the flowchart's AST, so it's a dummy placeholder
    // Only place where we can initialise a new StackFrame because this is only going to be called
    // during visualisation where stack frame info is not needed
  ...
  public FlowNode existingNode(ParseTree parseTree) {
    return nodes.stream()
        .filter(n -> n.getExecutionContext() == parseTree)
        .findFirst()
        .orElse(null);
  }
