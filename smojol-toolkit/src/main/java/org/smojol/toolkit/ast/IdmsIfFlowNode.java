...
public class IdmsIfFlowNode extends CobolFlowNode {
  ...
  public IdmsIfFlowNode(
      ParseTree parseTree,
      FlowNode scope,
      FlowNodeService nodeService,
      StackFrames staticFrameContext) {
  ...
  public void buildInternalFlow() {
    ...
    ParseTree ifThen =
        navigator.findByCondition(
            executionContext, n -> n.getClass() == CobolParser.IfThenContext.class, 1);
    ifThenBlock = nodeService.node(ifThen, this, staticFrameContext);
    ifThenBlock.buildFlow();
    ParseTree ifElse =
        navigator.findByCondition(
            executionContext, n -> n.getClass() == CobolParser.IfElseContext.class, 1);
    if (ifElse != null) {
  ...
  private ParseTree conditionContext(ParseTree searchRoot) {
    ...
    ParseTree idmsContainer =
        navigator.findByCondition(
            searchRoot, n -> n.getClass() == IdmsParser.IdmsIfStatementContext.class);
  ...
  public List<FlowNode> astChildren() {
    return ifElseBlock != null
        ? ImmutableList.of(ifThenBlock, ifElseBlock)
        : ImmutableList.of(ifThenBlock);
  }
