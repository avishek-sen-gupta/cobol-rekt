...
public class FlowchartBuilder {
  ...
  private void buildChartGraphic(
      Function<VisitContext, Boolean> stopCondition, FlowNode root, ChartOverlay chartOverlay) {
  ...
  private void write(String dotFilePath) {
    try {
      Graphviz.fromGraph(graph).engine(Engine.DOT).render(Format.DOT).toFile(new File(dotFilePath));
    } catch (IOException e) {
