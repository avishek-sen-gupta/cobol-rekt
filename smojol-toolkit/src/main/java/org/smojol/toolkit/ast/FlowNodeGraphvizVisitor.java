...
public class FlowNodeGraphvizVisitor implements FlowNodeVisitor {
  ...
  public FlowNodeGraphvizVisitor(
      MutableGraph g, ChartOverlay overlay, Function<VisitContext, Boolean> stopCondition) {
    this.g = g;
  ...
  public void visit(
    ...
    List<FlowNode> targets =
        outgoingNodes.stream().map(FlowNode::passthrough).map(overlay::block).toList();
    ...
    if (node.accessesDatabase()) {
      g.add(
          flowchartSource.addLink(
              existingOrNew("IDMS Database")
                  .add("shape", "cylinder")
                  .add("height", "4")
                  .add("width", "4")
                  .add("style", "filled")
  ...
  public void visitParentChildLink(
    ...
    visitParentChildLink(
        parent, internalTreeRoot, visitContext, nodeService, FlowNodeCondition.ALWAYS_SHOW);
  }
  ...
  public void visitParentChildLink(
    ...
    MutableNode graphParent =
        styled(overlayParent, existingOrNew(overlayParent.id()))
            .add("label", overlayParent.label());
    MutableNode graphChild =
        existingOrNew(overlayInternalTreeRoot.id()).add("label", overlayInternalTreeRoot.label());
    MutableNode child = styled(overlayInternalTreeRoot, graphChild);
    //        if (overlayInternalTreeRoot.getExecutionContext() != null &&
    // overlayInternalTreeRoot.getExecutionContext().getClass() ==
    // CobolParser.ConditionalStatementCallContext.class)
    String arrowStyle = hideStrategy.apply(overlayInternalTreeRoot) ? "none" : "normal";
    graphParent.addLink(
        graphParent.linkTo(child).with("style", "dashed").with("arrowhead", arrowStyle));
  }

  private MutableNode existingOrNew(String nodeID) {
    List<MutableNode> matchingNodes =
        g.nodes().stream().filter(gn -> gn.name().toString().equals(nodeID)).toList();
    ...
    if (matchingNodes.size() > 1) {
      throw new RuntimeException(
          String.format(
              "Duplicate node detected, %s copies found of ID: %s",
              matchingNodes.size(), matchingNodes.getFirst().name()));
    }
  ...
  public void visitControlTransfer(FlowNode from, FlowNode to, VisitContext visitContext) {
    ...
    MutableNode origin =
        styled(overlayFrom, existingOrNew(overlayFrom.id())).add("label", overlayFrom.label());
    MutableNode destination =
        styled(overlayTo, existingOrNew(overlayTo.id())).add("label", overlayTo.label());
  ...
  public void group(FlowNode root) {
    ...
    MutableGraph outliningCluster =
        mutGraph(clusterID).setCluster(true).graphAttrs().add("bgcolor", Color.LIGHTGREY.value);
    flowNodes.forEach(
        n -> {
