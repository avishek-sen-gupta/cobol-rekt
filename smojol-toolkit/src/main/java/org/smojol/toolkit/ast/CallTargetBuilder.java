...
public class CallTargetBuilder {
  public static CallTarget target(CobolParser.CallStatementContext callStmt) {
    if (callStmt.constantName() != null)
      return new StaticCallTarget(callStmt.constantName().getText());
  ...
  static CallTarget target(IdmsParser.TransferStatementContext transfer) {
    if (transfer.idms_program_name() != null)
      return new StaticCallTarget(transfer.idms_program_name().getText());
