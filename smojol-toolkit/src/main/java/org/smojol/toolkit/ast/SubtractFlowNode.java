...
public class SubtractFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.SubtractStatementContext subtractStatement =
        new SyntaxIdentity<CobolParser.SubtractStatementContext>(executionContext).get();
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    if (!isGiving()) {
      List<CobolExpression> lhsExpressions =
          minuends.stream().map(lhs -> builder.identifier(lhs.generalIdentifier())).toList();
      List<CobolExpression> rhsExpressions =
          subtrahends.stream()
              .map(rhs -> builder.literalOrIdentifier(rhs.literal(), rhs.generalIdentifier()))
              .toList();
      ...
      CobolExpression lhsGivingExpressions =
          builder.literalOrIdentifier(minuendsGiving.literal(), minuendsGiving.generalIdentifier());
      List<CobolExpression> rhsGivingExpressions =
          subtrahendsGiving.stream()
              .map(rhs -> builder.literalOrIdentifier(rhs.literal(), rhs.generalIdentifier()))
              .toList();
      List<CobolExpression> givingDestinationExpressions =
          givingDestinations.stream()
              .map(dest -> builder.identifier(dest.generalIdentifier()))
              .toList();
