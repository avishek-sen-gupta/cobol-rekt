...
public class EvaluateFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.EvaluateStatementContext whenStatement =
        new SyntaxIdentity<CobolParser.EvaluateStatementContext>(executionContext).get();
    evaluationChannels.add(whenStatement.evaluateSelect());
    evaluationChannels.addAll(
        whenStatement.evaluateAlsoSelect().stream()
            .map(CobolParser.EvaluateAlsoSelectContext::evaluateSelect)
            .toList());
    whenPhrases =
        whenStatement.evaluateWhenPhrase().stream()
            .map(ewp -> new EvaluateBranchFlowNode(ewp, this, nodeService, staticFrameContext))
            .toList();

    //        CobolParser.EvaluateStatementContext whenStatement = new
    // SyntaxIdentity<CobolParser.EvaluateStatementContext>(getExecutionContext()).get();
    deconstructedRepresentation =
        new EvaluateBreaker(staticFrameContext, this, nodeService).decompose(whenStatement);
  ...
  @Override
  public String name() {
    CobolParser.EvaluateStatementContext evaluateStatement =
        new SyntaxIdentity<CobolParser.EvaluateStatementContext>(getExecutionContext()).get();
    return "EVALUATE";
  }
