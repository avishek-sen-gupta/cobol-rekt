...
public class FlowNodeOverlayVisitor implements FlowNodeVisitor {
  ...
  // TODO: This needs to become idempotent. There will be duplicate groups if the mergeable items do
  // not come from higher up the syntax tree first.
  // TODO: The condition needs to be refactored, probably do not need to be so picky about which
  // types, and just check for isMergeable(), need to test
  // Currently, this works because super.buildInternalFlow() is called before
  // internalTreeRoot.buildFlow() in CompositeCobolNode.
  @Override
  public void visit(
      FlowNode node,
    ...
    if ((node.getClass() == SentenceFlowNode.class
            ...
            || (MergingChartOverlay.isAtomic(node)
                && !contained(
                    node)) // This condition is a little sus because technically, statements inside
        // sentences could also get their own groups which would show up in
        // addition to their parent sentence groups. It's working now, need to
        // investigate with a small test program.
        )
        && node.isMergeable()) {
