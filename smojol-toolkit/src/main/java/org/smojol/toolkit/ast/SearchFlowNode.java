...
public class SearchFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.SearchStatementContext searchStatementContext =
        new SyntaxIdentity<CobolParser.SearchStatementContext>(executionContext).get();
    ...
    whenPhrases =
        searchWhenContexts.stream()
            .map(when -> nodeService.node(when, this, staticFrameContext))
            .toList();
  ...
  public void acceptUnvisited(FlowNodeVisitor visitor, int level) {
    ...
    whenPhrases.forEach(
        w -> visitor.visitParentChildLink(this, w, new VisitContext(level), nodeService));
  ...
  public String label() {
    ...
    //        return CobolContextAugmentedTreeNode.originalText(executionContext,
    // CobolEntityNavigator::PASSTHROUGH);
  }
