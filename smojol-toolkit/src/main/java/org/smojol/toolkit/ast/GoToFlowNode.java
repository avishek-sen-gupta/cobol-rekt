...
public class GoToFlowNode extends CobolFlowNode implements InternalControlFlowNode {
  ...
  public void buildControlFlow() {
    CobolParser.GoToStatementContext goToStatement =
        new SyntaxIdentity<CobolParser.GoToStatementContext>(getExecutionContext()).get();
    List<CobolParser.ProcedureNameContext> procedureNames = goToStatement.procedureName();
    logger.finer(
        "Found a GO TO, routing to "
            + String.join(",", procedureNames.stream().map(RuleContext::getText).toList()));
    destinationNodes =
        procedureNames.stream()
            .map(p -> nodeService.sectionOrParaWithName(p.paragraphName().getText()))
            .toList();
    if (dependsUponFactor())
      dependingFactor = new CobolExpressionBuilder().identifier(goToStatement.generalIdentifier());
  }
  ...
  public void acceptUnvisited(FlowNodeVisitor visitor, int level) {
    ...
    destinationNodes.forEach(
        destinationNode ->
            visitor.visitControlTransfer(this, destinationNode, new VisitContext(level)));
  }
