...
public class PerformInlineFlowNode extends CompositeCobolFlowNode {
  ...
  public void buildInternalFlow() {
    ...
    //        bodyStatements = x.conditionalStatementCall().stream().map(stmt ->
    // nodeService.node(stmt, this, staticFrameContext)).toList();
    if (isVarying(x)) {
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    nestedLoops =
        FlowIterationBuilder.build(performInlineStatementContext.performType(), dataStructures);
