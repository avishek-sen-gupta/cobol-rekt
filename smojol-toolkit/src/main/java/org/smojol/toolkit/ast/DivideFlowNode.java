...
public class DivideFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.DivideStatementContext divideStatement =
        new SyntaxIdentity<CobolParser.DivideStatementContext>(executionContext).get();
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    divisorExpression =
        builder.literalOrIdentifier(intoDivisor.literal(), intoDivisor.generalIdentifier());
    dividendExpressions =
        dividends != null
            ? dividends.stream()
                .map(dividend -> builder.identifier(dividend.generalIdentifier()))
                .toList()
            : ImmutableList.of(
                builder.literalOrIdentifier(
                    givingDividend.literal(), givingDividend.generalIdentifier()));
    ...
    if (divideStatement.divideIntoStatement() != null) {
      ...
      dividendExpressions =
          divideStatement.divideIntoStatement().divideInto().stream()
              .map(lhs -> builder.identifier(lhs.generalIdentifier()))
              .toList();
      divisorExpression =
          builder.literalOrIdentifier(
              divideStatement.divisor().literal(), divideStatement.divisor().generalIdentifier());
      ...
      dividendExpressions =
          ImmutableList.of(
              builder.literalOrIdentifier(
                  divideStatement.divideIntoGivingStatement().literal(),
                  divideStatement.divideIntoGivingStatement().generalIdentifier()));
      destinationExpressions =
          divideStatement.divideIntoGivingStatement().divideGivingPhrase().divideGiving().stream()
              .map(dst -> builder.identifier(dst.generalIdentifier()))
              .toList();
      ...
      dividendExpressions =
          ImmutableList.of(
              builder.literalOrIdentifier(
                  divideStatement.divisor().literal(),
                  divideStatement.divisor().generalIdentifier()));
      destinationExpressions =
          divideStatement.divideByGivingStatement().divideGivingPhrase().divideGiving().stream()
              .map(dst -> builder.identifier(dst.generalIdentifier()))
              .toList();
