...
public class CobolFlowNodeFactory {
  public static FlowNode newNode(
    ...
    if (SyntaxIdentity.isStatementOfType(parseTree, CobolParser.IfStatementContext.class))
      ...
      return new StopFlowNode(parseTree, scope, nodeService, stackFrames);
        ...
        || SyntaxIdentity.isOfType(
            parseTree, CobolParser.NextSentenceWrapperStatementContext.class))
    ...
    else if (SyntaxIdentity.isStatementOfType(
        parseTree, CobolParser.GenericOnClauseStatementContext.class))
      ...
      return new AddFlowNode(parseTree, scope, nodeService, stackFrames);
        ...
        parseTree, CobolParser.SubtractStatementContext.class))
      return new SubtractFlowNode(parseTree, scope, nodeService, stackFrames);
    else if (SyntaxIdentity.isStatementOfType(
        parseTree, CobolParser.MultiplyStatementContext.class))
    ...
    else if (SyntaxIdentity.isOfType(
        parseTree, CobolParser.ConditionalStatementCallContext.class)) {
      CobolParser.StatementContext innerVanillaStatement =
          ((CobolParser.ConditionalStatementCallContext) parseTree).statement();
      CobolParser.DialectStatementContext innerDialectStatement =
          ((CobolParser.ConditionalStatementCallContext) parseTree).dialectStatement();
      ParseTree innerStatement =
          innerVanillaStatement != null ? innerVanillaStatement : innerDialectStatement;
      return newNode(innerStatement, scope, nodeService, stackFrames);
      //            return new ConditionalStatementFlowNode(parseTree, scope, nodeService,
      // stackFrames);
    }
    ...
    if (SyntaxIdentity.isOfType(parseTree, CobolParser.StatementContext.class))
      ...
      return new StopFlowNode(parseTree, scope, nodeService, stackFrames);
        ...
        || SyntaxIdentity.isOfType(
            parseTree, CobolParser.NextSentenceWrapperStatementContext.class))
      return new NextSentenceFlowNode(parseTree, scope, nodeService, stackFrames);
    //        else if (parseTree instanceof CobolParser.PerformStatementContext p &&
    // SyntaxIdentity.satisfies(parseTree, SyntaxIdentity::PERFORM_PROCEDURE))
    else if (parseTree instanceof CobolParser.PerformStatementContext p
        && p.performProcedureStatement() != null)
      return new PerformProcedureFlowNode(parseTree, scope, nodeService, stackFrames);
    else if (parseTree instanceof CobolParser.PerformStatementContext p
        && p.performInlineStatement() != null)
      return new PerformInlineFlowNode(parseTree, scope, nodeService, stackFrames);
      ...
      return DialectFlowNodeFactory.flowNode(parseTree, scope, nodeService, stackFrames);
        ...
        parseTree, CobolParser.ConditionalStatementCallContext.class)) {
      CobolParser.StatementContext innerStatement =
          ((CobolParser.ConditionalStatementCallContext) parseTree).statement();
      return newNode(innerStatement, scope, nodeService, stackFrames);
      //            return new ConditionalStatementFlowNode(parseTree, scope, nodeService,
      // stackFrames);
  ...
  private static boolean isCompositeNode(ParseTree executionContext) {
    ...
    return executionContext.getClass() == CobolParser.DialectSectionContext.class
