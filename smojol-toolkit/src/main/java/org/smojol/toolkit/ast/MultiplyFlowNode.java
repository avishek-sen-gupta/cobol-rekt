...
public class MultiplyFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.MultiplyStatementContext multiplyStatement =
        new SyntaxIdentity<CobolParser.MultiplyStatementContext>(executionContext).get();
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    rhsExpressions =
        rhses.stream().map(rhs -> builder.identifier(rhs.generalIdentifier())).toList();
    if (givingRhs != null)
      givingRhsExpression =
          builder.literalOrIdentifier(givingRhs.literal(), givingRhs.generalIdentifier());
    givingDestinationExpressions =
        givingDestinations.stream()
            .map(rhs -> builder.identifier(rhs.generalIdentifier()))
            .toList();
