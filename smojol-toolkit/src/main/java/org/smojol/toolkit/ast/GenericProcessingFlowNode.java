...
public class GenericProcessingFlowNode implements FlowNode {
  @Getter private final Class<? extends FlowNode> type;
  private List<FlowNode> nodes = new ArrayList<>();
  private final FlowNode enclosingScope;
  private String uuid;

  public GenericProcessingFlowNode(
      FlowNode node, FlowNode enclosingScope, FlowNodeService nodeService) {
  ...
  public String label() {
    ...
    nodes.forEach(
        n ->
            builder
                .append(NodeText.originalText(n.getExecutionContext(), NodeText::PASSTHROUGH))
                .append("\n"));
