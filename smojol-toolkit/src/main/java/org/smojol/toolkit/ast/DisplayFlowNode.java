...
public class DisplayFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.DisplayStatementContext displayStatement =
        new SyntaxIdentity<CobolParser.DisplayStatementContext>(executionContext).get();
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    operandExpressions =
        operands.stream()
            .map(op -> builder.literalOrIdentifier(op.literal(), op.generalIdentifier()))
            .toList();
  }
}
