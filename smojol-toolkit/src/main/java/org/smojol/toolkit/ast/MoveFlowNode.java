...
public class MoveFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.MoveStatementContext moveStatement =
        new SyntaxIdentity<CobolParser.MoveStatementContext>(executionContext).get();
    fromSingle =
        Optional.of(moveStatement)
            .map(CobolParser.MoveStatementContext::moveToStatement)
            .map(CobolParser.MoveToStatementContext::moveToSendingArea)
            .orElse(null);
    tos =
        moveStatement.moveToStatement() != null
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    if (moveStatement.moveToStatement() != null) {
      CobolParser.MoveToSendingAreaContext sendingArea =
      ...
      // TODO: Maybe distribute this across multiple expressions, one corresponding to each
      // destination, but with a separate type
      fromExpressions =
          ImmutableList.of(
              sendingArea.literal() != null
                  ? builder.literal(sendingArea.literal(), expectedType)
                  : builder.identifier(sendingArea.generalIdentifier()));
    } else {
      CobolParser.MoveCorrespondingToSendingAreaContext sendingArea =
          moveStatement.moveCorrespondingToStatement().moveCorrespondingToSendingArea();
      fromExpressions = ImmutableList.of(builder.identifier(sendingArea.generalIdentifier()));
    }
  }

  private static List<CobolExpression> toExpressions(
      CobolParser.MoveStatementContext moveStatement, CobolExpressionBuilder builder) {
    if (moveStatement.moveToStatement() != null)
      return moveStatement.moveToStatement().generalIdentifier().stream()
          .map(builder::identifier)
          .toList();
    else
      return moveStatement.moveCorrespondingToStatement().generalIdentifier().stream()
          .map(builder::identifier)
          .toList();
  }
}
