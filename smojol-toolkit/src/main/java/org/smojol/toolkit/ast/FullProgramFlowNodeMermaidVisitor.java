...
public class FullProgramFlowNodeMermaidVisitor implements FlowNodeVisitor {
  ...
  private static String node(FlowNode node) {
    return switch (node.type()) {
      ...
      case SECTION, PROCEDURE_DIVISION_BODY ->
          String.format("%s[\"%s\"]", node.id(), escaped(node.name()));
  ...
  public void visitParentChildLink(
    ...
    if (parent.type() == FlowNodeType.IF_BRANCH)
      lines.add(
          directedLabelledParentChildEdge(
              parent, child, child.type() == FlowNodeType.IF_YES ? "YES" : "NO"));
