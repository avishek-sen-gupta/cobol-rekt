...
public class CobolFlowNode implements FlowNode {
  ...
  public String toString() {
    if (executionContext instanceof ParserRuleContext) {
      return name()
          + "/"
          + ((ParserRuleContext) executionContext).getStart().getLine()
          + "/"
          + uuid;
    }
    return name() + "." + uuid;
  ...
  public String name() {
    if (executionContext.getClass() == CobolParser.SectionOrParagraphContext.class) {
      CobolParser.SectionOrParagraphContext ctx =
          (CobolParser.SectionOrParagraphContext) executionContext;
    ...
    if (executionContext.getClass() == TerminalNodeImpl.class) return executionContext.getText();
    ...
    if (executionContext.getClass() == CobolParser.ParagraphsContext.class) return "para-group:";
  ...
  protected String truncated(ParseTree e, int truncationLimit) {
    ...
    //        return e.getText().length() > truncationLimit ? e.getText().substring(0,
    // truncationLimit) : e.getText();
  }
  ...
  protected CobolVmSignal continueOrAbort(
      CobolVmSignal defaultSignal, CobolInterpreter interpreter, FlowNodeService nodeService) {
  ...
  protected CobolVmSignal next(
      CobolVmSignal signal, CobolInterpreter interpreter, FlowNodeService nodeService) {
