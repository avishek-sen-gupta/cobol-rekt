...
public class SentenceFlowNode extends CompositeCobolFlowNode {
  ...
  public boolean isMergeable() {
    ...
    long unmergeableStatementCount =
        e.statement().stream()
            .filter(stmt -> !nodeService.existingNode(stmt).isMergeable())
            .count();
  ...
  // TODO: Why does the recursive search through the outgoingNodes of the internalTreeRoot collapse
  // the sourceGraph?
  ...
  protected CobolVmSignal continueOrAbort(
    ...
    if (defaultSignal == CobolVmSignal.NEXT_SENTENCE)
      return next(CobolVmSignal.CONTINUE, interpreter, nodeService);
