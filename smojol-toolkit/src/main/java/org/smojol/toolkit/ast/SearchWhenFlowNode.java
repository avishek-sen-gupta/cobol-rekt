...
public class SearchWhenFlowNode extends CompositeCobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.SearchWhenContext searchWhenStatementContext =
        (CobolParser.SearchWhenContext) executionContext;
  ...
  public void acceptUnvisited(FlowNodeVisitor visitor, int level) {
    ...
    // Condition is already shown in parent's label, so we don't need to explicitly visit (and
    // create) a condition sourceGraph node
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    conditionExpression =
        new CobolExpressionBuilder()
            .condition(searchWhenStatementContext.condition(), nodeService.getDataStructures());
    whenFlowNodes =
        searchWhenStatementContext.conditionalStatementCall().stream()
            .map(stmt -> nodeService.node(stmt, this, staticFrameContext))
            .toList();
