...
public class CompositeCobolFlowNode extends CobolFlowNode {
  ...
  public static FlowNodeCondition CHILD_IS_CONDITIONAL_STATEMENT =
      node ->
          SyntaxIdentity.isOfType(
              node.getExecutionContext(), CobolParser.ConditionalStatementCallContext.class);
  protected FlowNode internalTreeRoot;

  public CompositeCobolFlowNode(
  ...
  public void acceptUnvisited(FlowNodeVisitor visitor, int level) {
    // super() call needs to happen first, otherwise duplicate overlays will be created, since
    // children will get GenericProcessingNodes earlier than parents.
  ...
  @Override
  public void acceptUnvisited(
      FlowNodeVisitor visitor, FlowNodeCondition stopRecurseCondition, int level) {
  ...
  public FlowNode next(FlowNodeCondition nodeCondition, FlowNode startingNode, boolean isComplete) {
    ...
    //        boolean shouldSearch = navigator.findByCondition(executionContext, n -> n ==
    // startingNode.getExecutionContext()) == null;
    if (!isComplete) {
    ...
    FlowNode chainSearch =
        new FlowNodes(outgoingNodes, nodeService).first().next(nodeCondition, startingNode, false);
    if (chainSearch != null) return chainSearch;
    return scope != null
        ? scope.next(nodeCondition, startingNode, true)
        : new DummyFlowNode(nodeService, staticFrameContext);
  }
  ...
  public CobolVmSignal acceptInterpreterForCompositeExecution(
      CobolInterpreter interpreter, FlowControl flowControl) {
    return executeInternalRoot(interpreter, nodeService);
  }

  protected CobolVmSignal executeInternalRoot(
      CobolInterpreter interpreter, FlowNodeService nodeService) {
    interpreter.enter(this, nodeService);
    CobolVmSignal signal =
        internalTreeRoot != null
            ? internalTreeRoot.acceptInterpreter(interpreter.scope(this), FlowControl::CONTINUE)
            : CobolVmSignal.CONTINUE;
