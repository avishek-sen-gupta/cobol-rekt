...
public class AddFlowNode extends CobolFlowNode {
  ...
  public void buildInternalFlow() {
    CobolParser.AddStatementContext addStatement =
        new SyntaxIdentity<CobolParser.AddStatementContext>(executionContext).get();
  ...
  public void resolve(SmojolSymbolTable symbolTable, CobolDataStructure dataStructures) {
    ...
    fromExpressions =
        froms.stream()
            .map(from -> builder.literalOrIdentifier(from.literal(), from.generalIdentifier()))
            .toList();
    tosGivingExpressions =
        tosGiving.stream()
            .map(from -> builder.literalOrIdentifier(from.literal(), from.generalIdentifier()))
            .toList();
