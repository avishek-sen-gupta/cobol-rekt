...
public class SmojolTasks {
  ...
  public SmojolTasks(
      ...
      Neo4JDriverBuilder neo4JDriverBuilder) {
  ...
  public List<AnalysisTaskResult> run(List<CommandLineAnalysisTask> commandLineAnalysisTasks)
      throws IOException {
    return tasks(commandLineAnalysisTasks).map(AnalysisTask::run).toList();
  }

  public AnalysisTask FLOW_TO_NEO4J =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new InjectIntoNeo4JTask(
                  baseModel.flowRoot(),
  ...
  public AnalysisTask ATTACH_COMMENTS =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new AttachCommentsTask(
                  baseModel.flowRoot(),
                  baseModel.dataStructures(),
                  baseModel.navigator(),
                  sourceConfig)
  ...
  public AnalysisTask WRITE_DATA_STRUCTURES =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new WriteDataStructuresTask(
                  baseModel.dataStructures(), dataStructuresOutputConfig, resourceOperations)
              .run();
        }
      };

  public AnalysisTask FLOW_TO_GRAPHML =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new ExportToGraphMLTask(
                  baseModel.flowRoot(),
                  baseModel.dataStructures(),
                  graphMLOutputConfig,
                  qualifier,
  ...
  public AnalysisTask COMPARE_CODE =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new CompareCodeTask(
                  baseModel.flowRoot(),
                  baseModel.dataStructures(),
                  qualifier,
  ...
  public AnalysisTask EXPORT_UNIFIED_TO_JSON =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new ExportUnifiedModelToJSONTask(
                  baseModel.flowRoot(),
                  baseModel.dataStructures(),
                  qualifier,
  ...
  public AnalysisTask WRITE_RAW_AST =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new WriteRawASTTask(baseModel.navigator(), rawAstOutputConfig, resourceOperations)
              .run();
        }
      };
  ...
  public AnalysisTask WRITE_FLOW_AST =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new WriteFlowASTTask(baseModel.flowRoot(), flowASTOutputConfig, resourceOperations)
              .run();
        }
      };

  public AnalysisTask BUILD_TRANSPILER_FLOWGRAPH =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          TranspilerFlowgraph transpilerFlowgraph =
              new BuildTranspilerFlowgraphTask(
                      baseModel.rawAST(),
                      baseModel.dataStructures(),
                      baseModel.symbolTable(),
                      ImmutableList.of("MAIN-SECTION-01", "SUBROUTINE-1"),
                      transpilerModelOutputConfig,
                      resourceOperations,
                      neo4JDriverBuilder)
                  .run();
          return AnalysisTaskResult.OK(
              CommandLineAnalysisTask.BUILD_TRANSPILER_FLOWGRAPH, transpilerFlowgraph);
        }
      };

  public AnalysisTask DRAW_FLOWCHART =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new DrawFlowchartTask(
                  baseModel.navigator(),
  ...
  public AnalysisTask EXPORT_MERMAID =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new ExportMermaidTask(
                  baseModel.flowRoot(), mermaidOutputConfig, resourceOperations)
              .run();
        }
      };

  public AnalysisTask WRITE_CFG =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new WriteControlFlowGraphTask(
                  baseModel.flowRoot(), idProvider, cfgOutputConfig, resourceOperations)
              .run();
        }
      };

  public AnalysisTask BUILD_PROGRAM_DEPENDENCIES =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
  ...
  public AnalysisTask SUMMARISE_THROUGH_LLM =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
  ...
  public AnalysisTask WRITE_LLM_SUMMARY =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
          return new WriteLLMSummaryTask(
                  baseModel.flowRoot(),
                  baseModel.dataStructures(),
                  llmOutputConfig,
                  resourceOperations)
  ...
  public AnalysisTask BUILD_BASE_ANALYSIS =
      new AnalysisTask() {
        @Override
        public AnalysisTaskResult run() {
  ...
  private Stream<AnalysisTask> tasks(List<CommandLineAnalysisTask> commandLineAnalysisTasks) {
    return commandLineAnalysisTasks.stream()
        .map(
            t ->
                switch (t) {
