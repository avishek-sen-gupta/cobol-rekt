...
public class SqliiteAccessTest {
  ...
  public void canAccessDB() throws SQLException {
    ...
    try (Connection conn = DriverManager.getConnection(url, user, password)) {
      ...
      int result = using.insertInto(PROJECT).columns(field("NAME")).values("PROJECT-1").execute();
      JumpIfTranspilerNode transpilerNode =
          new JumpIfTranspilerNode(
              new NamedLocationNode("ABCD"),
              new EqualToNode(
                  new ValueOfNode(new SymbolReferenceNode("A")),
                  new PrimitiveValueTranspilerNode(TypedRecord.typedNumber(10))));
      String json = new Gson().toJson(transpilerNode);
      JsonObject jsonObject = new Gson().fromJson(json, JsonObject.class);
      Table<Record> IR_AST = table("IR_AST");
      Result<Record1<String>> names =
          using.select(IR_AST.field("PROGRAM_NAME", String.class)).from(IR_AST).fetch();
      System.out.println("NAMESSSSS>>>>>>>>>>>>>>>>>>>>>");
      System.out.println(names);
      Result<Record4<String, String, Integer, Integer>> allIntermediateASTs =
          using
              .select(
              ...
              .from(IR_AST)
              .join(PROJECT)
              .on(field("IR_AST.PROJECT_ID", Integer.class).eq(field("PROJECT.ID", Integer.class)))
              .fetch();

      //            List<Map<String, Object>> collectedIntermediateASTs = new
      // intermediateASTListingsByProject(allIntermediateASTs);
