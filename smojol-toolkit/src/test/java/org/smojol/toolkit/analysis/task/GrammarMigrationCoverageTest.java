...
/**
 * Exercises all grammar constructs that changed between COBOL LSP 2.2.0 and 2.4.3. See
 * COBOL-LSP-INTEGRATION.md Section 4 for the migration patterns tested here.
 *
 * <p>Grammar constructs covered: 1. sectionOrParagraph (replaces procedureSection) - numeric and
 * named sections 2. tableCall argument wrapper - multi-dimensional array subscripts 3.
 * performUntilCondition intermediate rule - PERFORM UNTIL 4. functionReference intermediate rule -
 * FUNCTION LENGTH 5. pictureString singular accessor - PIC clauses 6. CobolParser (merged from
 * CobolDataDivisionParser) - data division parsing
 */
class GrammarMigrationCoverageTest {
  ...
  private SerialisableASTFlowNode buildFlowAST() throws IOException {
    AnalysisTaskResult taskResult =
        new TestTaskRunner("grammar-constructs.cbl", "test-code/grammar-coverage")
