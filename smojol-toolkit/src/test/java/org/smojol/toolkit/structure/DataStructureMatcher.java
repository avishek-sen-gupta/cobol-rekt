...
public class DataStructureMatcher {
  ...
  DataStructureMatcher(
      Function1<CobolDataStructure, SelfStructureMatchResult> selfMatcher,
      DataStructureMatcher... childMatchers) {
  ...
  public StructureMatchResult match(CobolDataStructure rec) {
    ...
    if (!selfMatchResult.matched())
      return new StructureMatchResult(selfMatchResult, ImmutableList.of());
    ...
    if (childMatchers.size() != rec.subStructures().size())
      return new StructureMatchResult(
          new SelfStructureMatchResult(
              false,
              ImmutableList.of(
                  new StructurePropertyMatchResult(
                      false,
                      String.format(
                          "Sizes of matcher (%s) and structures (%s) in record '%s' do not match!",
                          childMatchers.size(), rec.subStructures().size(), rec.name())))),
          ImmutableList.of());
    Stream<ImmutablePair<DataStructureMatcher, CobolDataStructure>> matcherNodePairs =
        zip(childMatchers.stream(), rec.subStructures().stream(), ImmutablePair::of);
    return new StructureMatchResult(
        selfMatchResult, matcherNodePairs.map(p -> p.getLeft().match(p.getRight())).toList());
  }

  public static DataStructureMatcher conditional(String conditionalName, CobolDataType dataType) {
    return new DataStructureMatcher(
        recordDefinition -> {
          List<StructurePropertyMatchResult> structurePropertyMatchResults =
              ImmutableList.of(
                  mustMatch(
                      ConditionalDataStructure.class,
                      recordDefinition.getClass(),
                      recordDefinition,
                      "staticStructure"),
                  mustMatch(
                      false, recordDefinition.isRedefinition(), recordDefinition, "isRedefinition"),
                  mustMatch(conditionalName, recordDefinition.name(), recordDefinition, "name"),
                  mustMatch(
                      dataType, recordDefinition.getDataType(), recordDefinition, "dataType"));
        ...
        recordDefinition -> {
          List<StructurePropertyMatchResult> structurePropertyMatchResults =
              ImmutableList.of(
                  ...
                  mustMatch(
                      AbstractCobolType.OBJECT,
                      recordDefinition.getDataType().abstractType(),
                      recordDefinition,
                      "abstractType"));
          return new SelfStructureMatchResult(true, structurePropertyMatchResults);
        },
    ...
    return new DataStructureMatcher(
        recordDefinition ->
            new SelfStructureMatchResult(
                true,
                ImmutableList.of(
                    mustBeTrue(recordDefinition != null, recordDefinition, "notNull"))),
        matchers.length > 0,
        matchers);
  }

  public static DataStructureMatcher format1(
      String recordName,
      AbstractCobolType abstractDataType,
      boolean isRedefinition,
      DataStructureMatcher... matchers) {
    return new DataStructureMatcher(
        recordDefinition -> {
          List<StructurePropertyMatchResult> structurePropertyMatchResults =
              ImmutableList.of(
                  mustMatch(
                      Format1DataStructure.class,
                      recordDefinition.getClass(),
                      recordDefinition,
                      "type"),
                  mustMatch(
                      isRedefinition,
                      recordDefinition.isRedefinition(),
                      recordDefinition,
                      "isRedefinition"),
                  mustMatch(recordName, recordDefinition.name(), recordDefinition, "name"),
                  mustMatch(
  ...
  public static DataStructureMatcher group(
      String recordName, boolean isRedefinition, DataStructureMatcher... matchers) {
    return new DataStructureMatcher(
        recordDefinition -> {
          List<StructurePropertyMatchResult> structurePropertyMatchResults =
              ImmutableList.of(
  ...
  public static DataStructureMatcher format1(
      String recordName, AbstractCobolType abstractDataType, DataStructureMatcher... matchers) {
  ...
  public static StructurePropertyMatchResult mustBeTrue(
      boolean condition, CobolDataStructure recordDefinition, String conditionDescription) {
    return condition
        ? new StructurePropertyMatchResult(true, "OK")
        : new StructurePropertyMatchResult(
            false,
            String.format(
                "Expected condition '%s' on record '%s' to be true but was false",
                conditionDescription, recordDefinition.name()));
  }

  public static StructurePropertyMatchResult mustMatch(
      Object expected, Object actual, CobolDataStructure recordDefinition, String field) {
    boolean matched = expected.equals(actual);
    return new StructurePropertyMatchResult(
        matched,
        matched
            ? "OK"
            : String.format(
                "Expected field '%s' of record '%s' to be %s but was %s",
                field, recordDefinition.name(), expected, actual));
  }

  public static DataStructureMatcher table(
      String recordName, int occurrences, DataStructureMatcher... matchers) {
  ...
  public static DataStructureMatcher table(
    ...
    return new DataStructureMatcher(
        recordDefinition -> {
          ImmutableList<StructurePropertyMatchResult> structurePropertyMatchResults =
              ImmutableList.of(
                  mustMatch(
                      TableDataStructure.class,
                      recordDefinition.getClass(),
                      recordDefinition,
                      "typeMustBeTable"),
                  mustMatch(
                  ...
                  mustMatch(
                      occurrences,
                      recordDefinition.subStructures().size(),
                      recordDefinition,
                      "elementSize"));
          return new SelfStructureMatchResult(true, structurePropertyMatchResults);
        },
